...
def writeStruct(out, arch, struct, title="Structure"):
    ...
    for chip in struct["description"]:
        ...
        if (not "CN" in struct_items) and (not chip_family in struct_items):
            struct[chip_family] = csr_utils.createSubset(arch, chip_family, struct)
            struct_items[chip_family] = buildStructMembers(chip_family, struct[chip_family])
...
def writeReg(out, arch, reg):
    ...
    out.write("{\n")
    address_items = {}
    for chip in csr_data.iterChipOrder(reg["address"]):
        is_this_arch = csr_utils.isChipArch(arch, chip)
        if not is_this_arch:
            continue
    ...
    for i in range(chip_list.MAX_CSR_PARAMS):
        if i < num_args:
    ...
    if "CN" in address_items:
        ...
        for chip in csr_data.iterChipOrder(address_items):
            ...
            if address_items[chip][0] == None: # No param check
                out.write("    if (bdk_is_model(%s))\n" % chip_name)
            else: # Need param check
                out.write("    if (bdk_is_model(%s) && (%s))\n" % (chip_name, address_items[chip][0]))
...
def writeMain(arch, blocks):
    ...
    writeBanner(out)
    out.write("\n")
    out.write("extern void __bdk_csr_fatal(const char *name, int num_args")
    for i in range(chip_list.MAX_CSR_PARAMS):
        out.write(", unsigned long arg%d" % (i+1))
    out.write(") __attribute__ ((noreturn));\n")
    out.write("\n")
