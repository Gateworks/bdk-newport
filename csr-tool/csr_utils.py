...
# architectures are "octeon", "thunder".
#
def isChipArch(arch, chip):
    assert arch in ["octeon", "thunder", "octeontx"], "Invalid architecture"
    if chip.startswith("CN9"):
        return (arch == "octeontx")
    elif chip.startswith("CNF9"):
        return (arch == "octeontx")
    elif chip.startswith("CN88"):
        return (arch == "thunder")
        ...
        return (arch == "octeon")
    elif chip.startswith("LOKI_"):
        return (arch == "octeontx")
    elif chip.startswith("TITAN_"):
        return (arch == "octeontx")
    else:
        assert False, "Invalid chip " + chip
...
# on error. Output is a tuple. First is the name with ranges replaced with
...
#
def addressToString(address_list, range_list, addSuffix=False, smartHex=True, useMasks=True):
    if isinstance(address_list, StringType):
        return address_list
    suffix = ""
    if addSuffix:
    ...
    for i,v in enumerate(address_list):
        param = chr(ord('a') + i - 1)
        if useMasks and (i > 0) and (range_list !=None):
...
#
# Compare an item for two different chips. When this is called the
...
def isDifferent(chip1, chip2, data):
    ...
    for key in data:
        ...
        if key == "ranges":
            continue
        # Allow differences in description
        if key == "description":
            continue
        item = data[key]
...
#
# Given the fields of a structure or register, fill in the subset FAMILY
...
def createSubset(arch, family, struct_reg):
    ...
    for name in subset_fields:
        ...
        for n in subset_fields:
            ...
            if start_bit <= subset_fields[n]["bits"][family][1] and stop_bit >= subset_fields[n]["bits"][family][0]:
