...
def writeReg(out, chip, reg):
    ...
    if reg["bar_size"][chip]:
        out.write("<tr>" + TH("Bar Size") + TD("%d bits" % reg["bar_size"][chip]) + "</tr>\n")
        address_eq = csr_utils.addressToString(reg["bar_offset"][chip], reg["bar_ranges"][chip])
...
def writeObject(chip, name, obj):
    ...
    out = open("html/%s-%s.html" % (chip.lower(), display_name.lower()), "w")
    ...
    for b in block_list:
        ...
        for name in names:
            if not isinstance(block[name], csr_data.Enum):
                continue
            display_name = name.replace("#", "x").upper()
            link = "<a href=%s-%s.html>%s</a>" % (chip.lower(), display_name.lower(), display_name)
            out.write("        %s<br>\n" % link)
...
def writeChip(chip, enums, structs, regs):
    out = open("html/%s.html" % chip.lower(), "w")
...
def diffObject(chip1, chip2, name, obj):
    display_name = name.replace("#", "x").upper()
    out = open("html/%s-%s-%s.html" % (chip1.lower(), chip2.lower(), display_name.lower()), "w")
        ...
        def do_objects(obj_type):
            for name in names:
                ...
                if not chip1_exists and not chip2_exists:
                    ...
                    # Deleted from chip2
                    link = "<a class=diff_sub href=%s-%s.html>%s(Removed)</a>" % (chip1.lower(), display_name.lower(), display_name)
                elif not chip1_exists and chip2_exists:
                    # Added to chip2
                    link = "<a class=diff_add href=%s-%s.html>%s(Added)</a>" % (chip2.lower(), display_name.lower(), display_name)
                elif csr_utils.isDifferent(chip1, chip2, block[name]):
                    # Different between the chips
                    link = "<a href=%s-%s-%s.html>%s(Changed)</a>" % (chip1.lower(), chip2.lower(), display_name.lower(), display_name)
...
def diffChip(chip1, chip2, enums, structs, regs):
    filename = "%s-%s.html" % (chip1.lower(), chip2.lower())
...
def writeMainIndex(enums, structs, regs):
    ...
    for model in models:
        ...
        for chip, p in by_model[model]:
            link = "<a href=%s.html target=\"frame\">%.1f</a>" % (chip.lower(), p)
            out.write(TD(link))
            if last_chip:
