...
void __bdk_trust_init(void)
{
    ...
    const int CSIB_S_FS0_WORD = 4;
    const int CSIB_S_FS_LEN_BYTES = 32;
    bdk_add_img_attestation_info("BOOT.BIN", 0,
        (uint8_t *)&__bdk_trust_csib.u[CSIB_S_FS0_WORD], CSIB_S_FS_LEN_BYTES);
success_trust:
...
{
    /* Count leading zeros in FUSF_CTL[ROM_T_CNT] to determine the
       hardware NV value */
