...
#endif /* SWIG */
    uint  (*sbus_func_ptr)  (void *aapl, uint addr, unsigned char reg_addr, unsigned char command, unsigned int sbus_data, int recv_data_back);
    void *client_data;
...
EXT uint  (*aapl_user_supplied_log_func_ptr)  (const char  *funcNamePtr, char  *fileNamePtr, unsigned int  lineNum, int  returnCode, char  *strMsgPtr);
#else
EXT uint  (*aapl_user_supplied_log_func_ptr)  (const char  *funcNamePtr, char  *strMsgPtr);
