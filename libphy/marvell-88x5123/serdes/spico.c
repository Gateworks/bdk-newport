    ...
    uint sbus_addr) /**< [in] SBus address of SerDes */
{
    ...
    case AVAGO_PROCESS_F:
        ...
        {
           pc = avago_sbus_rd(aapl, sbus_addr, 0x25);
           interrupt = avago_sbus_rd(aapl, sbus_addr, 0x4);
           mem_bist = avago_sbus_rd(aapl, sbus_addr, 0x9);
           stepping = avago_sbus_rd(aapl, sbus_addr, 0x20);
           enable = avago_sbus_rd(aapl, sbus_addr, 0x7);
           error = avago_sbus_rd(aapl, sbus_addr, 0x2a);
        ...
        {
            pc = avago_sbus_rd(aapl, sbus_addr, 0xa);
            stepping = avago_sbus_rd(aapl, sbus_addr, 0x05);
            mem_bist = avago_sbus_rd(aapl, sbus_addr, 0x0);
            enable = avago_sbus_rd(aapl, sbus_addr, 0x1);
            error = avago_sbus_rd(aapl, sbus_addr, 0xf);
    ...
    int param)      /**< [in] Interrupt data */
{
...
#endif /* MV_HWS_REDUCED_BUILD */
    {
        ...
        {
            sbus_addr = avago_struct_to_addr(&next);
...
#endif /*  MV_HWS_REDUCED_BUILD */
void spico_burst_upload(Aapl_t *aapl, uint sbus, uint reg, uint rom_size, const short *rom)
{
    uint word;
    int data_to_write[3];
...
int spico_upload_image(Aapl_t *aapl, uint sbus_addr, int words, const short rom[])
{
    int return_code = aapl->return_code;
    int data_to_write;
