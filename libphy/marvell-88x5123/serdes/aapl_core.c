...
/* somehow and exit(). */
    ...
    AAPL_MEMSET(aapl, 0, sizeof(*aapl));
...
void aapl_init(Aapl_t *aapl)
{
    ...
    int i;
#endif /* MV_HWS_REDUCED_BUILD_EXT_CM3 */
    AAPL_MEMSET(aapl, 0, sizeof(Aapl_t));
#ifndef MV_HWS_REDUCED_BUILD_EXT_CM3
    for( i = 0; i < AAPL_MAX_CHIPS; i++ )
        aapl->chip_name[i] = aapl->chip_rev[i] = "";
...
#else
        aapl_fail(aapl, __func__, __LINE__, "Broadcast address (0x%02x) not supported.\n", sbus_addr);
#endif
...
#else
        aapl_fail(aapl, __func__, __LINE__, "Unsupported firmware rev 0x%04X on SBus address %s.\n",
            aapl_get_firmware_rev(aapl, sbus_addr), aapl_addr_to_str(sbus_addr));
#endif
...
#endif /* MV_HWS_REDUCED_BUILD */
{
    ...
    if( !match && error_on_no_match )
        ...
        aapl_fail(aapl, caller, line, "SBus %s, of IP type 0x%x, in process %s, is not supported by %s.\n",
            ...
            caller);
#else
    aapl_fail(aapl, __func__, __LINE__, "SBus %s, of IP type 0x%x, is not supported by %s.\n",
            aapl_addr_to_str(sbus_addr),
            aapl_get_ip_type(aapl,sbus_addr),
            aapl_get_process_id_str(aapl,sbus_addr),
            __func__);
...
#if defined(MV_HWS_REDUCED_BUILD_EXT_CM3) && defined(MV_HWS_AAPL_DEBUG_PRINT_ENABLE)
#include "printf.h"
#endif
