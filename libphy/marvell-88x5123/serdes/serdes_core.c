    ...
    uint data)                      /**< [in] Data to write into memory location addr. */
{
    ...
    case AVAGO_LSB_DIRECT:
        ...
        return;
#if 0
/* unreachable code - leave it as this is Avago Code */
        if(aapl_get_lsb_rev(aapl,addr) >= 3)
        {
            if(!( (aapl_get_process_id(aapl,addr) == AVAGO_PROCESS_B) || (aapl_get_ip_type(aapl,addr) == AVAGO_M4) ))
              avago_sbus_wr(aapl, addr, 0x02, (0<<31) | ((mem_addr & 0x1ff) << 16) | (data & 0xffff));
...
static int spico_int_02_retry(Aapl_t *aapl, uint addr, int data, int retry)
{
    ...
    aapl_fail(aapl,__func__,__LINE__,"SBus %s, PRBS reconfigure timed out.\n",aapl_addr_to_str(addr));
    return -1;
}
    ...
    long user_data[4])          /**< [in] 80-bit user data (4 x 20). */
{
    ...
    if( rc != 0x18 )
    {
        aapl_fail(aapl,__func__,__LINE__,"SBus %s, spico_int(0x18,0) returned 0x%x\n",aapl_addr_to_str(addr),rc);
        return -1;
    }
    rc = 1;
    for( i = 0; i < 4; i++ )
    ...
    if( !rc )
    {
        aapl_fail(aapl,__func__,__LINE__,"SBus %s, spico_int(0x19) returned 0x%x\n",aapl_addr_to_str(addr),rc);
        return -1;
    }
    ...
    int *rx)            /**< [out] Where to store RX width. */
{
...
#else
    reg_width = 0x301; reg_type = AVAGO_ESB; tx_shift = 0; rx_shift =  3;
#endif
    ...
    if( sdrev == AAPL_SDREV_16 || sdrev == AAPL_SDREV_HVD6 )
    ...
    uint addr)          /**< [in] Device address number. */
{
    ...
    if( aapl_get_ip_type(aapl,addr) == AVAGO_M4 )
        ret = (avago_serdes_mem_rd(aapl, addr, AVAGO_LSB, 0x22) & 0x40) ? TRUE: FALSE;
    AAPL_LOG_PRINT5(aapl, AVAGO_DEBUG5,__func__,__LINE__,"SBus %s, ip_type %d, return = %s\n", aapl_addr_to_str(addr), aapl_get_ip_type(aapl,addr), aapl_bool_to_str(ret));
    return ret;
}
    ...
    )
{
    ...
    if( bits < 0 )
    {
        aapl_fail(aapl, __func__, __LINE__, "ERROR: SBus %s, invalid width(s) %d/%d requested.\n",aapl_addr_to_str(addr),tx,rx);
        return -1;
    }
    if( (build_id & 2) == 0 )
        bits |= 0x8800;
    ...
    uint addr)          /**< [in] Device address number. */
{
    int compare_reg;
#ifndef MV_HWS_REDUCED_BUILD
    compare_reg = aapl_get_sdrev(aapl,addr) == AAPL_SDREV_P1 ? 0xe1 : 0x17;
#else
    compare_reg = 0x17;
#endif
...
uint divider_lookup28nm(uint data)
{
    uint divider;
    switch( data & 0xFF )
    {
