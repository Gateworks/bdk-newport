...
/* External references for library function, at both Host and FW.
   In firmware, APIs are imported by HWS, in each other case the
   native library functions can be used. */
...
*
* DESCRIPTION:
*       AVAGO log glud function. This function will build a perliminary log message
*       and send it to HWS log handler.
*
* INPUTS:
*       caller  - function name, or 0, depends on Avago code
*       line    - line number, or 0, depends on Avago code
*       fmt     - format string
*       ap      - arg list
*       log_sel - log level
*
...
*
* COMMENTS:
*       1. When running as FW, message strings will not be evaluated with the additional arguments,
*       i.e, ("msg %d", msgIntegerParam) will be evaluated to "msg" %d" and not "msg <msgIntegerParamas>"
*       the lack of the memory in FW arcitecture, and the fact that Avago code build it's own log messages.
*******************************************************************************/
#ifndef  MV_HWS_FREE_RTOS
...
#ifndef  MV_HWS_FREE_RTOS
            vsnprintf(buf, (HWS_STRING_BUF_SIZE-1), fmt, ap);
            aapl_user_supplied_log_func_ptr(caller/* function name */,
                        fileName,
                        line  /* line number */,
   ...
   which are function name and the format string */
        ...
        /* In FW side we log only error messages, no space for unimportant messages */
...
#endif /* MICRO_INIT */
            {
                ...
                osMemCpy( &buf[0], &fmt[0], (fmtSize > HWS_STRING_BUF_SIZE) ? HWS_STRING_BUF_SIZE : fmtSize );
#else
                /* Copying the format message as much as we can */
                fmtSize = 8;
                osMemCpy( &buf[0], "fail msg", (fmtSize > HWS_STRING_BUF_SIZE) ? HWS_STRING_BUF_SIZE : fmtSize );
#endif /* !(defined(MICRO_INIT) && defined(CM3)) */
                aapl_user_supplied_log_func_ptr(caller /* function name */,
                                         buf    /* string argument */);
            }
...
/********************************************************/
/**** Our private log handling declarations glue END ****/
/********************************************************/
...
#ifndef  MV_HWS_FREE_RTOS
    appl_log_user(caller, fileName, line, fmt, ap, AVAGO_ERR);
#else
    #if (defined(MICRO_INIT) && defined(CM3))
        appl_log_user(caller);
    #else
        appl_log_user(caller, fmt, AVAGO_ERR);
    #endif /* (defined(MICRO_INIT) && defined(CM3)) */
#endif /* MV_HWS_FREE_RTOS */
...
#endif /* MV_HWS_REDUCED_BUILD_EXT_CM3 */
{
    ...
    /* Calling our user defined log */
    #ifndef  MV_HWS_FREE_RTOS
        appl_log_user(caller, fileName, line, fmt, ap, log_sel);
    #else
        appl_log_user(caller, fmt, log_sel);
    #endif /* MV_HWS_FREE_RTOS */
