...
local configr8 = octeontx.c.bdk_pcie_config_read8
local configr16 = octeontx.c.bdk_pcie_config_read16
local configr32 = octeontx.c.bdk_pcie_config_read32
local configw8 = octeontx.c.bdk_pcie_config_write8
local configw16 = octeontx.c.bdk_pcie_config_write16
local configw32 = octeontx.c.bdk_pcie_config_write32
...
local function create_device(root, bus, deviceid, func, vparent)
    ...
    function newdev:getresources()
        ...
        if not self.isbridge then
            self:write32(PCICONFIG_ROM, 0xffffffff)
            local v = self:read32(PCICONFIG_ROM)
            v = 0xffffffff00000000 + v
            local size = bit64.bnot(bit64.band(v, -4)) + 1
            if size > 1 then
                table.insert(io_bars, {self, PCICONFIG_ROM, size, v})
            end
        end
            ...
            for _,bar in ipairs(bar_table) do
                ...
                if device then
                    if bar_table == io_bars then
                        ...
                        if bar_addr == PCICONFIG_ROM then
                            -- Set the enable bit
    ...
    function newdev:display(indent,show_vfs)
        ...
        if not self.vparent then
            ...
            if not self.isbridge then
                local rom = self:read32(PCICONFIG_ROM)
                local isen = bit64.btest(rom, 1)
                local rom_info = isen and "Enabled" or "Disabled"
                printf("%s    Expansion ROM:%08x [%s]\n", indent, rom, rom_info)
            end
        end
    ...
    if newdev.isbridge then
        ...
        if newdev:read8(PCICONFIG_SECONDARY_BUS) == 0 then
            ...
            -- Set to max value as we don't know how many buses are there
        ...
        -- Update the last bus number now that we know how many buses
        -- were there
...
--
function pcie.initialize(node, pcie_port, skip_init)
    -- Initialize the PCIe link. Only do this for real PCIe ports.
    -- The internal ECAMs are at a fake port of 100+, so skip init
    -- on those
    if (pcie_port < 100) and not skip_init then
        if octeontx.c.bdk_pcie_rc_initialize(node, pcie_port) ~= 0 then
    ...
    function pcie_root:scan()
        ...
        if pcie_port < 100 then
            ...
            if octeontx.is_model(octeontx.CN8XXX) then
                self.last_bus = octeontx.csr[self.node].PCIERCX_CFG006(self.port).SBNUM
            else
                self.last_bus = octeontx.csr[self.node].PCIERCX_BNUM(self.port).SBNUM
            end
            max_device = 0
        else
    ...
    function pcie_root:shutdown()
        local status = octeontx.c.bdk_pcie_rc_shutdown(self.node, self.port)
        if status ~= 0 then
