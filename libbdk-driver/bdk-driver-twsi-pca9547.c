...
BDK_REQUIRE_DEFINE(DRIVER_TWSI_PCA9547);
...
static void add(bdk_device_t *device, const char *config_str)
{
    ...
    BDK_TRACE(DEVICE, "%s: PCA9547 on TWSI=%s, ADDR=%d, CHAN=%d\n",
...
{
    int reg = (select < 0) ? 0 : ((select & 0x7) | 0x08);
...
bdk_driver_t __bdk_driver_twsi_pca9547 = {
