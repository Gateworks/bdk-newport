...
#if HAS_FLOAT
	int sec = time_in_secs(total_time);
	int msec = ((int)(time_in_secs(total_time) * 1000)) % 1000;
	ee_printf("Total time (secs): %d.%03d\n",sec, msec);
...
#if HAS_FLOAT
		if (known_id==3) {
			ee_printf("CoreMark 1.0 : %d / %s %s",(int)(default_num_contexts*results[0].iterations/time_in_secs(total_time)),COMPILER_VERSION,COMPILER_FLAGS);
