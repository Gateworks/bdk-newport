...
SUPPORTED_TESTS = {
    ...
    "lbk":          test_trafficgen.check_lbk_links,
    "qlm-detect":   test_qlm.qlm_detect,
    "qlm-config-all":test_qlm.qlm_config_all,
}
...
def parse_args():
    ...
    group.add_argument('--bmc', action='store', type=str,
        help='BMC to use for control. Only used with boards supporting IPMI and ASIM. ' +
        'Either this option, or "--mcu" must be specified. Set to "asim" when using ' +
        "Asim for simulation.")
    ...
    try:
        ...
        while count < args.count:
            ...
            for power in [True, False, False]:
                if (cnx.board == "asim") and not power:
                    continue # Asim doesn't support resets
