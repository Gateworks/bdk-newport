...
ATF_NONSECURE_FLASH_ADDRESS = int(os.getenv("ATF_NONSECURE_FLASH_ADDRESS", "0x400000"), 0) # defaults to 4MB
assert ATF_NONSECURE_FLASH_ADDRESS & 0xfffff == 0, "ATF_NONSECURE_FLASH_ADDRESS must be on a 1MB boundary"
FATFS_SIZE = int(ATF_NONSECURE_FLASH_ADDRESS) - FIXED_SIZE # 2.5MB of FAT filesystem data, see fatfs_create()
...
def flash_create(args):
    ...
    # Must be exactly 192KB for CN8XXX compatibility, not encrypted
    ...
    # Must be exactly 192KB for CN8XXX compatibility, encrypted
    ...
    # Must be 192KB or less, not encrypted
    mcp_bl1 = read_elf(args.mcp_bl1, 0x30000, False, 0x80004000)
    flash = update_image(flash, mcp_bl1, False, 0x10800, 0x10900, 0x100000, args)
    # Update the MCP BL1 trusted, CLIB=0x10a00, CSIB=0x10b00, Image=0x130000
    # Must be 192KB or less, encrypted
    flash = update_image(flash, mcp_bl1, True, 0x10a00, 0x10b00, 0x130000, args)
    # 0x160000: 128KB reserved for serdes firmware
    # 0x180000: Add the FAT filesystem to it
...
def flash_fdisk(filename):
    ...
    end_sector = (FIXED_SIZE + FATFS_SIZE) / 512 - 1
    ...
    inp += "%d\n" % end_sector #    End at sector 8191, 4MB
    inp += "t\n"    #    t          Change type
    inp += "1\n"    #    1          FAT12
    inp += "w\n"    #    w          Write changes
...
def generate_csib(crypt_type, image, nv):
    ...
    # Word 26-27
    csib += struct.pack("<Q", 0) * 2 # Software name / UUID
    # Word 28-31
...
#
def generate_clib(csib, offset, size):
    # Word 0
...
#
def sign_blob(blob):
    # Write the data to be signed into a file
    out = open("csib.tmp", "wb")
    out.write(blob)
    out.close()
    # Use bdk-sign to create the signature using ROT key
    assert os.system(BDK_ROOT + "/bin/bdk-sign hw-rot-private csib.tmp.sign csib.tmp") == 0, "Signing failed"
    # Read the signature
    inf = open("csib.tmp.sign", "rb")
    asn1 = inf.read()
    inf.close()
    # Convert the ASN1(DER) signature into binary. AES padding will have added
    # some junk onto the end of the signature. Ignore the padding
    asn1, padding = ecdsa.der.remove_sequence(asn1)
    # ASN1 should contain two integers and nothing else
    r, asn1 = ecdsa.der.remove_integer(asn1) # Python long integer
    s, asn1 = ecdsa.der.remove_integer(asn1) # Python long integer
    assert len(asn1) == 0, "Extra data in signature"
    r = binascii.unhexlify("%064x" % r) # Big endian binary
    s = binascii.unhexlify("%064x" % s) # Big endian binary
    assert len(r) == 32, "Extra data in R binary"
    assert len(s) == 32, "Extra data in S binary"
    # Cleanup the temporary files
    os.unlink("csib.tmp")
    os.unlink("csib.tmp.sign")
...
#
def read_pub_key(public_key_file):
    inf = open(public_key_file, "rb")
    pem = inf.read()
    inf.close()
...
def update_image(flash, image, encrypt, clib_loc, csib_loc, image_loc, args):
    ...
    clib = generate_clib(csib, image_loc, len(image))
...
def do_arg_parse():
    ...
    group.add_option("--key", action="store", type="string", dest="key",
                      nargs=1, default="hw-rot-public.pem",
                      help="Input Public Key file matching the private key used for 'hw-rot-private'")
