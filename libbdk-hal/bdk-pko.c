...
static int __bdk_pko_allocate_fifo(bdk_node_t node, int lmac, int size)
{
    ...
    while (fifo < max_fifo)
    {
        /* Build a mask representing what this fifo would use */
...
retry_lower_divider:
    ...
    /* Burst exponent is limited to 4 bits encoded in -1 notation */
 ...
 */
int bdk_pko_transmit(bdk_if_handle_t handle, const bdk_if_packet_t *packet, int repeat_count)
{
    ...
    /* Build the two PKO command words we need */
