...
static int vnic_setup_rbdr(nic_t *nic)
{
    ...
    if (do_fill)
    {
        ...
        if (bdk_is_model(OCTEONTX_CN83XX)) fill_num = fill_num/3; /* CN83XX has more nics */
    ...
    while (1)
    {
        ...
        while (count < pending_count)
        {
            ...
            if (bdk_likely(cq_header->s.cqe_type == BDK_NIC_CQE_TYPE_E_RX))
                rbdr_doorbell += if_process_complete_rx(nic->node, &vnic_rbdr_state, cq_header, cq_header_le, true);
            else
...
int bdk_nic_port_init(bdk_if_handle_t handle, bdk_nic_type_t ntype, int lmac_credits)
{
    ...
    if (global_node_state[handle->node] == NULL)
    {
        ...
        int num_nic_vf = nic_pf_const1.s.vnics;
    ...
    /* VNIC setup requirements
       ...
       4) One RQ per physical port, many RQ may share RBDR/CQ
        ...
        4) One CQ is allocated per entire interface, using index 0
    ...
    if (has_rx_nic)  {
        /* CPI is the output of the above algorithm, this is used to lookup the
    ...
    */
    ...
    if ((has_tx_nic) && (-1 != nic_lmac_e)) {
        int credit;
        if ((BDK_NIC_TYPE_LBK == nic->ntype) && bdk_is_model(OCTEONTX_CN83XX) )
 ...
 */
int bdk_nic_transmit(bdk_if_handle_t handle, const bdk_if_packet_t *packet, int repeat_count)
{
    ...
    int needed = (packet->segments + 1) * (1 + repeat_count);
    if (nic->sq_available < needed + SQ_SLOP)
    {
    ...
    BDK_CSR_WRITE(nic->node, BDK_NIC_QSX_SQX_DOOR(nic->nic_vf, nic->sq),
        sent * (packet->segments + 1));
    BDK_TRACE(NIC, "%s: Transmit Doorbell %d\n", nic->handle->name, sent * (packet->segments + 1));

    /* Update our cached state */
    nic->sq_available -= sent * (packet->segments + 1);
    nic->sq_loc = loc;
