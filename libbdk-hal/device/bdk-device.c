...
static bdk_device_t device_list[MAX_DEVICES];
...
int bdk_device_add_driver(struct bdk_driver_s *driver)
{
    ...
    driver_list = driver;
    BDK_TRACE(DEVICE, "Added driver %s for %08x\n", driver->name, driver->id);
    return 0;
}
 ...
 */
static bdk_driver_t *lookup_driver(bdk_device_t *device)
{
    bdk_driver_t *drv = driver_list;
    while (drv)
    {
        if (drv->id && (drv->id == device->id))
        {
            device->driver = drv;
            return drv;
        }
...
static void populate_device(bdk_device_t *device)
{
    ...
    /* Set the master and memory enable bits */
    ...
    while (bar <= max_bar)
    {
        ...
        if (!is64)
        {
            BDK_TRACE(DEVICE_SCAN, "%s: BAR%d Disabled or EA bar skipped (0x%08lx)\n", device->name, bar_index, address);
            bar += 8;
            continue;
        }
        ...
        BDK_TRACE(DEVICE_SCAN, "%s: BAR%d 0x%lx/%d flags=0x%x\n",
    ...
    BDK_TRACE(DEVICE_SCAN, "%s: Walking PCI capabilities\n", device->name);
    int has_pcie = 0;
    ...
    while (cap_loc)
    {
        ...
        else if (cap_id == 0x14)
        {
            ...
            for (int entry = 0; entry < ea_cap_hdr.s.num_entries; entry++)
            {
                ...
                *ptr = ptr[-1];
#endif
                asm volatile ("" ::: "memory"); /* Needed by gcc to detect aliases on ea_entry */
    ...
    if (has_pcie)
    {
        BDK_TRACE(DEVICE_SCAN, "%s: Walking PCIe capabilities\n", device->name);
        cap_loc = 0x100;
        while (cap_loc)
        {
            uint32_t cap = bdk_ecam_read32(device, cap_loc);
            ...
            else if (cap_id == 0x10)
            {
                ...
                while (bar <= (cap_loc + 0x3c))
                {
                    ...
                    if (!is64)
                    {
                        BDK_TRACE(DEVICE_SCAN, "%s:        SR-IOV BAR%d Disabled or EA bar skipped (0x%08lx)\n", device->name, bar_index, address);
                        bar += 8;
                        continue;
                    }
                    ...
                    BDK_TRACE(DEVICE_SCAN, "%s:        SR-IOV BAR%d 0x%llx/%d flags=0x%lx\n",
...
/**
 * Allocate and return a new empty device
 *
 * @return Device memory, or NULL on failure
 */
static bdk_device_t *add_device(void)
{
 ...
 * @return Zero on success, negative on failure
 */
int bdk_device_add_ecam(bdk_node_t node, int ecam, int bus, int dev, int func)
{
    bdk_device_t *device = add_device();
    if (!device)
        return -1;
    ...
    if (drv)
        BDK_TRACE(DEVICE, "%s: Added device with driver %s\n", device->name, drv->name);
    else
...
/**
 * Called when a new device is detected in the configuration system
 *
 * @param config_str String describing the device
 *
 * @return Zero on success, negative on failure
 */
static int bdk_device_add_config(const char *config_str)
{
    bdk_device_t *device;
    char name[sizeof(device->name)];
    char type_name[32];

    /* config_str always looks like "NAME,type=DRIVER,..." */

    /* Extract NAME, before the first comma */
    const char *first_comma = strchr(config_str, ',');
    if (!first_comma)
        goto bad_config;
    size_t name_len = first_comma - config_str;
    if ((name_len == 0) || (name_len >= sizeof(device->name)))
        goto bad_config;
    strncpy(name, config_str, name_len);
    name[name_len] = 0;

    /* Extract DRIVER, between ",type=" and the second comma */
    const char *second_comma = strchr(first_comma + 1, ',');
    if (!second_comma)
        second_comma = first_comma + strlen(first_comma);
    if (strncmp(first_comma + 1, "type=", 5) != 0)
        goto bad_config;
    const char *type_ptr = first_comma + 6;
    size_t type_len = second_comma - type_ptr;
    if (type_len > sizeof(type_name) - 1)
        goto bad_config;
    strncpy(type_name, type_ptr, type_len);
    type_name[type_len] = 0;

    /* Lookup the driver for this DRIVER name */
    bdk_driver_t *drv = driver_list;
    while (drv)
    {
        if (strcmp(drv->name, type_name) == 0)
            break;
        drv = drv->next;
    }
    ...
    if (drv)
        ...
        BDK_TRACE(DEVICE, "%s: Added device without driver (%s)\n", device->name, type_name);

    if (drv && drv->add)
        drv->add(device, config_str);
    return 0;

bad_config:
    bdk_error("driver: Invalid config string %s\n", config_str);
    return -1;
}
...
/**
 * Load the DEVICES config item and populate the listed devices
 */
static void bdk_device_load_config(void)
{
    int blob_length = 0;
    const char *blob = bdk_config_get_blob(&blob_length, BDK_CONFIG_DEVICES);
    if (!blob)
        return;
...
/**
 * Probe and initialize and devices mot already done
 *
 * @return Zero on success, negative on failure
 */
static int probe_and_init(void)
{
    /* Probe all devices first */
    for (int i = 0; i < device_list_count; i++)
    {
        bdk_device_t *dev = &device_list[i];
        const bdk_driver_t *drv = dev->driver;
        if (drv == NULL)
            continue;
 ...
 */
bdk_device_t *bdk_device_lookup(bdk_node_t node, uint32_t id, int instance)
{
    for (int i = 0; i < device_list_count; i++)
    {
        bdk_device_t *dev = &device_list[i];
 ...
 * @return Device pointer, or NULL if the device isn't found
 */
bdk_device_t *bdk_device_lookup_by_name(const char *name)
{
    /* Allow match if name contains a parameter enclosed in parens */
    const char *paren = strchr(name, '(');
    if (!paren)
        paren = name + strlen(name);
    size_t name_len = paren - name;
    if ((name_len < 1) || (name_len >= sizeof(device_list[0].name)))
        bdk_fatal("Invalid device name passed while looking up device (%s)\n", name);

    for (int i = 0; i < device_list_count; i++)
    {
        bdk_device_t *dev = &device_list[i];
        if ((dev->name[name_len] == 0) &&
            (strncmp(dev->name, name, name_len) == 0))
            return dev;
    }
    BDK_TRACE(DEVICE, "No device found for %s\n", name);
    return NULL;
}
