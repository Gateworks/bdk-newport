 ...
 * @param sli_group The index of the PCIe port on the SLI is returned here. This is a sequential
...
{
    if (bdk_is_model(OCTEONTX_CN83XX) || bdk_is_model(OCTEONTX_CN81XX))
    {
        /* Only one SLI */
        *sli = 0;
        *sli_group = pcie_port;
...
static void __bdk_pcie_rc_initialize_config_space(bdk_node_t node, int pcie_port)
{
    ...
    if (bdk_is_altpkg(OCTEONTX_CN81XX) && (pcie_port == 0))
    {
        lme = 1;
        mlw = 1;
    }
    ...
    /* CFG554.PRV default changed from 16'h7ff to 16'h593. Should be
       safe to apply to CN81XX, and CN83XX */
    ...
    /* Errata PEM-21178 - Change the CFG[089-092] LxUTP and LxDTP defaults.
       Should be safe to apply to CN81XX, and CN83XX */
    ...
    /* Errata PEM-31375 PEM RSL accesses to PCLK registers can timeout
       during speed change. Change MRML_INT_LOCAL_TO[tovalue] to
       525us */
    BDK_CSR_MODIFY(c, node, BDK_MRML_INT_LOCAL_TO,
        c.s.tovalue = bdk_clock_get_rate(node, BDK_CLOCK_SCLK) * 525 / 1000000);
}
...
retry_speed:
    ...
    do
    {
        ...
        if (pciercx_cfg032.u == 0xffffffff)
            continue;
...
static int __bdk_pcie_rc_reset(bdk_node_t node, int pcie_port)
{
    ...
    BDK_CSR_INIT(pemx_cfg, node, BDK_PEMX_CFG(pcie_port));
    int is_dual = bdk_is_model(OCTEONTX_CN81XX) ? pemx_cfg.cn81xx.lanes4 : pemx_cfg.cn83xx.lanes8;
...
{
    BDK_CSR_INIT(pemx_cfg, node, BDK_PEMX_CFG(pcie_port));
    int host_mode = bdk_is_model(OCTEONTX_CN83XX) ? pemx_cfg.cn83xx.hostmd : 1;
    if (!host_mode)
    {
    ...
    /* Read link state */
    BDK_CSR_INIT(link_ctl, node, BDK_PCIERCX_CFG032(pcie_port));
...
static int pcie_rc_initialize(bdk_node_t node, int pcie_port)
{
    ...
    /* Make sure we aren't trying to setup a target mode interface in host
       mode. Sadly this bit is RAZ for CN81XX because the hardware
...
const __bdk_pcie_ops_t pcie_ops_cn8xxx = {
