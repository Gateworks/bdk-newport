...
static bdk_if_link_t __bdk_if_phy_get_twsi(bdk_node_t dev_node, int phy_addr)
{
    ...
    bdk_device_t *twsi_device = bdk_device_lookup(node, BDK_DEVICE_DEVID(BDK_PCC_DEV_IDL_E_MIO_TWS), twsi_bus);
    int64_t phy_status = bdk_device_twsi_read_ia(twsi_device, twsi_addr, 17, 2, twsi_ia_width);
    if (phy_status != -1)
    {
...
bdk_if_link_t __bdk_if_phy_get(bdk_node_t dev_node, int phy_addr)
{
    ...
    /* PHY address of -1 means there is no PHY and we should have never
        gotten here */
    if (phy_addr == -1)
        return result;
...
int bdk_if_phy_setup(bdk_node_t node)
{
    for (int bgx = 0; bgx < 4; bgx++)
    {
		...
		int phy_status_1 = bdk_mdio_read(node, mdio_bus, phy_addr, BDK_MDIO_PHY_REG_ID1);
	    	...
	    	BDK_TRACE(INIT, "Found PHY (id=0x%x) on mdio bus:0x%x addr:0x%x connected to QLM%d\n",phy_status_1, mdio_bus, mdio_addr, qlm);
	        ...
	        if (phy_status_1 == 0x0007)
                {
                    ...
                    {
	    		BDK_TRACE(INIT, "Programming VSC8514 PHY\n");
                        bdk_if_phy_vsc8514_setup(node, qlm, mdio_bus, mdio_addr);
                    }
                    else
                    {
	    		BDK_TRACE(INIT, "Programming VSC8574 PHY\n");
		        bdk_if_phy_vsc8574_setup(node, qlm, mdio_bus, mdio_addr);
                    }
                }
		/* Marvell */
	        if (phy_status_1 == 0x0141)
		{
	    		BDK_TRACE(INIT, "Programming MARVELL PHY\n");
