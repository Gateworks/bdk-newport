...
/**
 * Initialize a port for use. All memory is allocated dynamically
...
{
    int result = 0;
    if ((handle->flags & BDK_IF_FLAGS_ENABLED) == 0)
    {
        handle->flags |= BDK_IF_FLAGS_ENABLED;
        if (handle->netport)
            bdk_device_netport_set(handle->netport, BDK_QLM_MODE_DISABLED, 0, true, false);
        if (__bdk_if_ops[handle->iftype]->if_enable)
            result = __bdk_if_ops[handle->iftype]->if_enable(handle);
    }
    return result;
}
...
int bdk_if_disable(bdk_if_handle_t handle)
{
    ...
    {
        handle->flags &= ~BDK_IF_FLAGS_ENABLED;
        if (__bdk_if_ops[handle->iftype]->if_disable)
            result = __bdk_if_ops[handle->iftype]->if_disable(handle);
        if (handle->netport)
            bdk_device_netport_set(handle->netport, BDK_QLM_MODE_DISABLED, 0, false, false);
    }
    return result;
}
 ...
 */
int bdk_if_transmit(bdk_if_handle_t handle, bdk_if_packet_t *packet, int repeat_count)
{
    return __bdk_if_ops[handle->iftype]->if_transmit(handle, packet, repeat_count);
}
...
static void __bdk_if_link_poll(int unused1, void *unused2)
{
    ...
    while (1)
    {
        ...
        if (link_handle && !(link_handle->flags & BDK_IF_FLAGS_HIDDEN))
...
int bdk_if_alloc(bdk_if_packet_t *packet, int length)
{
    ...
    while (length > 0)
    {
        uint64_t buf = __bdk_if_global_ops.alloc(buf_size);
        if (bdk_unlikely(!buf))
        {
            bdk_if_free(packet);
            return -1;
        ...
        int size = buf_size;
        if (packet->segments == 0)
        {
            /* Rob trailing 256 bytes for potentially building NIX/PKO
               gather array We can get away with subtraction because
               buffers are aligned in memory */
...
int bdk_if_link_wait_all(uint64_t timeout_us)
{
    ...
    while (1)
    {
        ...
        {
            if (!(link_handle->flags & BDK_IF_FLAGS_HIDDEN))
            {
                /* Check link */
