...
/* This file implements interfaces connected to the BGX block introduced
    for CN8XXX. This block combines SGMII, XAUI, DXAUI, RXAUI, XFI,
    XLAUI, 10GBASE-KR, and 40GBASE-KR all into one interface */
...
typedef enum
{
    ...
    BGX_MODE_1000X, /* 1 lane, 1.250 Gbaud */
...
static void create_priv(bdk_node_t node, int interface, int index, bgx_priv_t *priv)
{
    ...
    bdk_qlm_modes_t qlm_mode = (qlm == -1) ? BDK_QLM_MODE_DISABLED : bdk_qlm_get_mode(node, qlm, 0);
    int num_lanes = bdk_qlm_get_lanes(node, qlm);
    int gbaud_mhz = (qlm == -1) ? 0 : bdk_qlm_get_gbaud_mhz(node, qlm, 0);
    ...
    switch (qlm_mode)
    {
        case BDK_QLM_MODE_SGMII:
            ...
            priv->num_port = num_lanes; /* one lane per port */
            /* For historical reasons we allow auto selection of 1000X is SGMII
               is configured with a TWSI based PHY */
            if ((priv->phy_address & BDK_IF_PHY_TYPE_MASK)== BDK_IF_PHY_TWSI)
                priv->mode = BGX_MODE_1000X;
            else
        ...
        case BDK_QLM_MODE_RXAUI:
            ...
            priv->num_port = num_lanes / 2; /* two lanes per port */
    ...
    if ((qlm != -1) && (bdk_is_model(OCTEONTX_CN81XX) ||
        (bdk_is_model(OCTEONTX_CN83XX) && (interface == 2))))
    {
        int num_ports = 0;
        int base_qlm = bdk_is_model(OCTEONTX_CN83XX) ? 5 : (qlm & 2);
        for (int dlm = base_qlm; dlm <= base_qlm + 1; dlm++)
        {
            int num_lanes = bdk_qlm_get_lanes(node, dlm);
            switch (bdk_qlm_get_mode(node, dlm, 0))
            {
                case BDK_QLM_MODE_SGMII:
                case BDK_QLM_MODE_1G_X:
                case BDK_QLM_MODE_XFI:
                case BDK_QLM_MODE_10G_KR:
                    num_ports += num_lanes; /* one lane per port */
                    break;
                case BDK_QLM_MODE_RXAUI:
                    num_ports += num_lanes / 2; /* two lanes per port */
                    break;
                case BDK_QLM_MODE_XAUI:
                case BDK_QLM_MODE_XLAUI:
                case BDK_QLM_MODE_40G_KR4:
...
static int bgx_setup_one_time(bdk_if_handle_t handle)
{
    ...
    switch (priv->mode)
    {
        ...
        case BGX_MODE_10G_KR:
            ...
            if ((bdk_is_model(OCTEONTX_CN81XX) && ((qlm == 1) || (qlm == 3))) ||
                (bdk_is_model(OCTEONTX_CN83XX) && (qlm == 6)))
            {
                switch (bdk_qlm_get_mode(handle->node, qlm - 1, 0))
                {
                    case BDK_QLM_MODE_SGMII:
                    case BDK_QLM_MODE_1G_X:
                    case BDK_QLM_MODE_XFI:
                    case BDK_QLM_MODE_10G_KR:
                    {
                        int num_lanes = bdk_qlm_get_lanes(handle->node, qlm - 1);
                        /* Index 2+ maps to lanes 2+ */
                        lane_to_sds = handle->index - 2 + num_lanes;
                        break;
                    }
                    case BDK_QLM_MODE_RXAUI:
    ...
    int fifo_size = bgxx_const.s.tx_fifosz;
    if (priv->num_port == 2)
        fifo_size /= 2;
    ...
    /* Program the transmit threshold to use the whole FIFO. Leave 256 bytes
       ...
       thresholds are limited to 10 bits */
...
static int sgmii_link(bdk_if_handle_t handle)
{
    ...
    BDK_CSR_MODIFY(c, handle->node, BDK_BGXX_GMP_PCS_MISCX_CTL(handle->interface, handle->index),
        ...
        c.s.mode = (priv->mode == BGX_MODE_1000X);
...
static int if_init(bdk_if_handle_t handle)
{
    ...
    if ((priv->mode != BGX_MODE_SGMII) && (priv->mode != BGX_MODE_1000X) && (priv->mode != BGX_MODE_QSGMII))
    {
...
static bdk_if_link_t if_link_get_sgmii(bdk_if_handle_t handle)
{
    ...
    int qlm = bdk_qlm_get(handle->node, BDK_IF_BGX, handle->interface, handle->index);
    int speed = bdk_qlm_get_gbaud_mhz(handle->node, qlm, 0) * 8 / 10;
...
static bdk_if_link_t if_link_get_xaui(bdk_if_handle_t handle)
{
    ...
    if (link_up)
    {
        ...
        result.s.up = 1;
        result.s.full_duplex = 1;
        uint64_t speed = bdk_qlm_get_gbaud_mhz(handle->node, qlm, 0);
        switch (priv->mode)
        {
            case BGX_MODE_SGMII:
            case BGX_MODE_1000X:
