...
typedef enum
{
    ...
    BDK_IF_FLAGS_ENABLED = 2, /* State of enable / disable of packets on the port */
    BDK_IF_FLAGS_HIDDEN = 4, /* Port is hidden from user by device tree */
} bdk_if_flags_t;

typedef enum
{
    BDK_IF_LED_LINK_SLOW,   /* Link amber, on for slower speeds */
    BDK_IF_LED_LINK_FAST,   /* Link green on for max speed, also blinks for ID */
    BDK_IF_LED_ACTIVITY,    /* Blinks when active */
    BDK_IF_LED_ID,          /* Blinks when port ID is requested */
    __BDK_IF_LED_MAX
} bdk_if_led_t;
...
typedef struct
{
    __bdk_if_stats_t rx;
    __bdk_if_stats_t tx;
    uint64_t rsfec_corrected_errors;    /* RS-FEC corrected errors */
    uint64_t rsfec_uncorrected_errors;  /* RS-FEC uncorrected errors */
    uint64_t rsfec_symbol_errors;       /* RS-FEC symbols with errors */
    uint64_t fec_corrected_errors;      /* FEC corrected errors */
    uint64_t fec_uncorrected_errors;    /* FEC uncorrected errors */
} bdk_if_stats_t;
...
typedef struct __bdk_if_port
{
    ...
    struct
    {
        bdk_device_t *device;
        int led;
    } led[__BDK_IF_LED_MAX];
    bdk_device_t *netport;          /* Device representing the network port */
    uint32_t netphy;                /* Handle to PHY implemented by NETPHY API */
...
typedef struct
{
    ...
    bdk_packet_ptr_t packet[BDK_IF_MAX_GATHER]; /* List of segments. Each has a physical address and length */
} bdk_if_packet_t;
...
typedef struct
{
    ...
    int (*if_transmit)(bdk_if_handle_t handle, const bdk_if_packet_t *packet, int repeat_count); /* TX a packet */
...
int bdk_if_phy_setup(bdk_node_t node);
...
extern int bdk_if_transmit(bdk_if_handle_t handle, bdk_if_packet_t *packet, int repeat_count);
