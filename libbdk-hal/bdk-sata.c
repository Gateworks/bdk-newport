 ...
 */
static int sata_initialize(bdk_node_t node, int controller, bool no_device)
{
    ...
    BDK_TRACE(SATA, "N%d.SATA%d: Initializing\n", node, controller);
    ...
    if (clb_pa == 0)
    {
        BDK_TRACE(SATA, "N%d.SATA%d: Allocating CLB\n", node, controller);
    ...
    if (fb_pa == 0)
    {
        BDK_TRACE(SATA, "N%d.SATA%d: Allocating FIS\n", node, controller);
    ...
    BDK_TRACE(SATA, "N%d.SATA%d: Enable FIS\n", node, controller);
    BDK_CSR_MODIFY(c, node, BDK_SATAX_UAHC_P0_CMD(controller),
        c.s.fre = 1);   /* FIS-receive enable */

    BDK_TRACE(SATA, "N%d.SATA%d: Power on and spin up\n", node, controller);
    ...
    if (no_device)
    {
        BDK_TRACE(SATA, "N%d.SATA%d: Skipping detect\n", node, controller);
    }
    else
    {
        ...
        BDK_TRACE(SATA, "N%d.SATA%d: Waitng for detect\n", node, controller);
        ...
        BDK_TRACE(SATA, "N%d.SATA%d: Detect complete\n", node, controller);
    }
    ...
    if (no_device)
    {
        BDK_TRACE(SATA, "N%d.SATA%d: Skipping device status\n", node, controller);
    }
    else
    {
        ...
        BDK_TRACE(SATA, "N%d.SATA%d: Waiting for device status\n", node, controller);
    ...
    BDK_TRACE(SATA, "N%d.SATA%d: Enabling AHCI command queuing\n", node, controller);
    ...
    BDK_TRACE(SATA, "N%d.SATA%d: Starting the controller\n", node, controller);
    BDK_CSR_MODIFY(c, node, BDK_SATAX_UAHC_P0_CMD(controller),
        c.s.st = 1); /* Start the controller */
    BDK_TRACE(SATA, "N%d.SATA%d: Initialization complete\n", node, controller);
    return 0;
...
{
    return sata_initialize(node, controller, false);
}
...
int bdk_sata_bist_fis(bdk_node_t node, int controller, int port, bdk_sata_bist_fis_t mode)
{
    if (!__bdk_sata_is_initialized(node, controller))
    {
        if (sata_initialize(node, controller, true))
            return -1;
    }
    ...
    /*
        Note from the Synopsis SATA bist training video on pattern generation
    ...
    */
    if (mode == BDK_SATA_BIST_SW_RETIMED)
    {
        ...
        bdk_warn("\n");
        bdk_warn("********************************************************\n");
        bdk_warn("* SATA retimed loopback will continue until chip\n");
        bdk_warn("* reset. No SATA menu options will affect the output.\n");
        bdk_warn("********************************************************\n");
        bdk_warn("\n");
        printf("\33[0m"); /* Normal */
        return 0;
    ...
    else if ((mode >= BDK_SATA_BIST_SW_TX_ONLY_SSOP) && (mode <= BDK_SATA_BIST_SW_TX_ONLY_LFTP))
    {
        ...
        BDK_CSR_MODIFY(c, node, BDK_SATAX_UAHC_GBL_BISTCR(controller),
            c.s.txo = 1;
            c.s.pattern = mode - BDK_SATA_BIST_SW_TX_ONLY_SSOP);
        BDK_TRACE(SATA, "N%d.SATA%d: Started tranmsit only\n", node, controller);

        printf("\33[1m"); /* Bold */
        bdk_warn("\n");
        bdk_warn("********************************************************\n");
        bdk_warn("* SATA pattern generation will continue until chip\n");
        bdk_warn("* reset. No SATA menu options will affect the output.\n");
        bdk_warn("********************************************************\n");
        bdk_warn("\n");
        printf("\33[0m"); /* Normal */
