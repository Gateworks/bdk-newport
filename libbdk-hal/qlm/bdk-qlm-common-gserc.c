...
#include "libbdk-arch/bdk-csrs-gserc.h"
 ...
 * @return State data
 */
bdk_qlm_state_lane_t __bdk_qlm_gserc_get_state(bdk_node_t node, int qlm, int lane)
{
    bdk_qlm_state_lane_t state = {.u = BDK_CSR_READ(node, BDK_GSERCX_SCRATCHX(qlm, lane))};
    return state;
}
 ...
 */
int __bdk_qlm_gserc_set_mode(bdk_node_t node, int qlm, int lane, bdk_qlm_modes_t mode, int baud_mhz, bdk_qlm_mode_flags_t flags)
{
    ...
    for (int l = start_lane; l <= end_lane; l++)
        BDK_CSR_WRITE(node, BDK_GSERCX_SCRATCHX(qlm, l), state.u);
    return 0;
}
 ...
 * @return Zero on success, negative on failure
 */
int __bdk_qlm_gserc_reset(bdk_node_t node, int qlm)
{
    BDK_CSR_INIT(gsercx_const, node, BDK_GSERCX_CONST(qlm));
    gsercx_const.s.nr_lanes = 2; // FIXME: Asim says 4
    for (int lane = 0; lane < gsercx_const.s.nr_lanes; lane++)
    {
        BDK_CSR_MODIFY(c, node, BDK_GSERCX_LANEX_CONTROL_BCFG(qlm, lane),
            c.s.ln_rst = 1);
    }
 ...
 * @return Zero on success, negative on failure
 */
int __bdk_qlm_gserc_enable_prbs(bdk_node_t node, int qlm, int prbs, bdk_qlm_direction_t dir)
{
    int pattern = 1;
    switch (prbs)
    {
    ...
    for (int lane = 0; lane < gsercx_const.s.nr_lanes; lane++)
    {
        ...
        {
            /* No error injection */
            BDK_CSR_WRITE(node, BDK_GSERCX_LNX_BIST_TX_BER_CTRL0(qlm, lane), 0);
            /* Seed PRBS */
            BDK_CSR_WRITE(node, BDK_GSERCX_LNX_BIST_TX_PRBS_CTRL0(qlm, lane), bdk_rng_get_random8());
            BDK_CSR_WRITE(node, BDK_GSERCX_LNX_BIST_TX_PRBS_CTRL1(qlm, lane), bdk_rng_get_random8());
            BDK_CSR_WRITE(node, BDK_GSERCX_LNX_BIST_TX_PRBS_CTRL2(qlm, lane), bdk_rng_get_random8());
            BDK_CSR_WRITE(node, BDK_GSERCX_LNX_BIST_TX_PRBS_CTRL3(qlm, lane), bdk_rng_get_random8());
            /* Start transmit */
            BDK_CSR_MODIFY(c, node, BDK_GSERCX_LNX_BIST_TX_CTRL(qlm, lane),
                c.s.pattern_sel = pattern;
                c.s.en = 1);
            /* Mux data from BIST generator */
            BDK_CSR_MODIFY(c, node, BDK_GSERCX_LNX_TOP_DPL_TXDP_CTRL1(qlm, lane),
                c.s.tx_dmux2_sel = 1);
        ...
        {
            /* Clear error counter */
            BDK_CSR_MODIFY(c, node, BDK_GSERCX_LNX_BIST_RX_CTRL(qlm, lane),
                c.s.stop_error_count = 1;
                c.s.clear_ber = 1);
            /* Allow re-lock */
            BDK_CSR_MODIFY(c, node, BDK_GSERCX_LNX_BIST_RX_LOSS_LOCK_CTRL4(qlm, lane),
 ...
 * @return Zero on success, negative on failure
 */
int __bdk_qlm_gserc_disable_prbs(bdk_node_t node, int qlm)
{
    BDK_CSR_INIT(gsercx_const, node, BDK_GSERCX_CONST(qlm));
    gsercx_const.s.nr_lanes = 2; // FIXME: Asim says 4
    for (int lane = 0; lane < gsercx_const.s.nr_lanes; lane++)
 ...
 */
uint64_t __bdk_qlm_gserc_get_prbs_errors(bdk_node_t node, int qlm, int lane, int clear)
{
    BDK_CSR_INIT(status, node, BDK_GSERCX_LNX_BIST_RX_STATUS(qlm, lane));
    ...
    BDK_CSR_INIT(status0, node, BDK_GSERCX_LNX_BIST_RX_BER_STATUS0(qlm, lane));
    BDK_CSR_INIT(status1, node, BDK_GSERCX_LNX_BIST_RX_BER_STATUS1(qlm, lane));
    BDK_CSR_INIT(status2, node, BDK_GSERCX_LNX_BIST_RX_BER_STATUS2(qlm, lane));
 ...
 * @param lane   Which lane
 */
void __bdk_qlm_gserc_inject_prbs_error(bdk_node_t node, int qlm, int lane)
{
    BDK_CSR_DEFINE(ctrl0, BDK_GSERCX_LNX_BIST_TX_BER_CTRL0(0, 0));
    BDK_CSR_DEFINE(ctrl1, BDK_GSERCX_LNX_BIST_TX_BER_CTRL1(0, 0));
    BDK_CSR_DEFINE(ctrl2, BDK_GSERCX_LNX_BIST_TX_BER_CTRL2(0, 0));
    BDK_CSR_DEFINE(ctrl3, BDK_GSERCX_LNX_BIST_TX_BER_CTRL3(0, 0));
    BDK_CSR_DEFINE(ctrl4, BDK_GSERCX_LNX_BIST_TX_BER_CTRL4(0, 0));
    BDK_CSR_DEFINE(ctrl5, BDK_GSERCX_LNX_BIST_TX_BER_CTRL5(0, 0));
    BDK_CSR_DEFINE(ctrl6, BDK_GSERCX_LNX_BIST_TX_BER_CTRL6(0, 0));
    ...
    BDK_CSR_WRITE(node, BDK_GSERCX_LNX_BIST_TX_BER_CTRL6(qlm, lane), ctrl6.u);
    BDK_CSR_WRITE(node, BDK_GSERCX_LNX_BIST_TX_BER_CTRL5(qlm, lane), ctrl5.u);
    BDK_CSR_WRITE(node, BDK_GSERCX_LNX_BIST_TX_BER_CTRL4(qlm, lane), ctrl4.u);
    BDK_CSR_WRITE(node, BDK_GSERCX_LNX_BIST_TX_BER_CTRL3(qlm, lane), ctrl3.u);
    BDK_CSR_WRITE(node, BDK_GSERCX_LNX_BIST_TX_BER_CTRL2(qlm, lane), ctrl2.u);
    BDK_CSR_WRITE(node, BDK_GSERCX_LNX_BIST_TX_BER_CTRL1(qlm, lane), ctrl1.u);
    BDK_CSR_WRITE(node, BDK_GSERCX_LNX_BIST_TX_BER_CTRL0(qlm, lane), ctrl0.u);
}
 ...
 * @return Zero on success, negative on failure
 */
int __bdk_qlm_gserc_enable_loop(bdk_node_t node, int qlm, bdk_qlm_loop_t loop)
{
    ...
    for (int lane = 0; lane < gsercx_const.s.nr_lanes; lane++)
    {
        BDK_CSR_MODIFY(c, node, BDK_GSERCX_LNX_TOP_AFE_LOOPBACK_CTRL(qlm, lane),
            c.s.loopback_nea_en = near_end_analog;
            c.s.loopback_fea_en = far_end_analog);
        BDK_CSR_MODIFY(c, node, BDK_GSERCX_LNX_TOP_DPL_RXDP_CTRL1(qlm, lane),
 ...
 * @return Zero on success, negative on failure
 */
int __bdk_qlm_gserc_tune_lane_tx(bdk_node_t node, int qlm, int lane, int tx_cmain, int tx_cpre, int tx_cpost, int tx_unused1, int tx_unused2)
{
    BDK_CSR_INIT(ctrl0, node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_CTRL0(qlm, lane));
    BDK_CSR_INIT(status0, node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_STATUS0(qlm, lane));
    if (ctrl0.s.req || status0.s.ack)
    {
        bdk_error("GSERC%d.%d: TXEQ coefficient request in progress\n", qlm, lane);
        return -1;
    }
    int orig_tx_cmain = -1;
    int orig_tx_cpre = -1;
    int orig_tx_cpost = -1;
    if (__bdk_qlm_gserc_get_tune_lane_tx(node, qlm, lane, &orig_tx_cmain, &orig_tx_cpre, &orig_tx_cpost, &tx_unused1, &tx_unused2))
    {
        bdk_error("GSERC%d.%d: Failed to read previous settings\n", qlm, lane);
        return -1;
    }
    ...
    /* Load the new value */
    BDK_CSR_MODIFY(c, node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_CTRL1(qlm, lane),
        c.s.txeq_c1 = tx_cpost);
    BDK_CSR_MODIFY(c, node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_CTRL3(qlm, lane),
        c.s.txeq_cm1 = tx_cpre);
    BDK_CSR_MODIFY(c, node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_CTRL5(qlm, lane),
        c.s.drv_swing = tx_cmain);
    /* Request an update */
    BDK_CSR_MODIFY(c, node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_CTRL0(qlm, lane),
        c.s.req = 1);
    /* Wait for ACK */
    if (BDK_CSR_WAIT_FOR_FIELD(node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_STATUS0(qlm, lane), ack, ==, 1, 10000))
    {
        bdk_error("GSERC%d.%d: TXEQ coefficient request timeout\n", qlm, lane);
        return -1;
    }
 ...
 * @return Zero on success, negative on failure
 */
int __bdk_qlm_gserc_get_tune_lane_tx(bdk_node_t node, int qlm, int lane, int *tx_cmain, int *tx_cpre, int *tx_cpost, int *tx_bs, int *tx_unused)
{
    BDK_CSR_INIT(status1, node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_STATUS1(qlm, lane));
    BDK_CSR_INIT(status2, node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_STATUS2(qlm, lane));
    BDK_CSR_INIT(status3, node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_STATUS3(qlm, lane));
 ...
 */
void __bdk_qlm_gserc_display_settings(bdk_node_t node, int qlm, int qlm_lane, bool show_tx, bool show_rx)
{
    printf("N%d.GSERC%d Lane %d:\n", node, qlm, qlm_lane);
    if (show_tx)
    {
        BDK_CSR_INIT(status1, node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_STATUS1(qlm, qlm_lane));
        BDK_CSR_INIT(status2, node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_STATUS2(qlm, qlm_lane));
        BDK_CSR_INIT(status3, node, BDK_GSERCX_LNX_DRV_REFCLK_TXEQ_STATUS3(qlm, qlm_lane));
    ...
    if (show_rx)
    {
        ...
        {
            case 0:
                att = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXLEQ_PLE_ATT_O_READ_VAL_2_0_RSVD(qlm));
                apg = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXLEQ_GN_APG_O_READ_VAL_2_0_RSVD(qlm));
                lfg = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXLEQ_EQ_LFG_O_READ_VAL_4_0_RSVD(qlm));
                hfg = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXLEQ_EQ_HFG_O_READ_VAL_4_0_RSVD(qlm));
                mbf = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXLEQ_EQ_MBF_O_READ_VAL_3_0_RSVD(qlm));
                mbg = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXLEQ_EQ_MBG_O_READ_VAL_3_0_RSVD(qlm));
                break;
            case 1:
                att = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXLEQ_PLE_ATT_O_READ_VAL_2_0_RSVD(qlm));
                apg = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXLEQ_GN_APG_O_READ_VAL_2_0_RSVD(qlm));
                lfg = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXLEQ_EQ_LFG_O_READ_VAL_4_0_RSVD(qlm));
                hfg = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXLEQ_EQ_HFG_O_READ_VAL_4_0_RSVD(qlm));
                mbf = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXLEQ_EQ_MBF_O_READ_VAL_3_0_RSVD(qlm));
                mbg = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXLEQ_EQ_MBG_O_READ_VAL_3_0_RSVD(qlm));
                break;
        }
        ...
        switch (qlm_lane)
        {
            case 0:
                if (use_even == 1)
                    tap1_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXDFE_EVEN_TAP1_O_READ_VAL_6_0_RSVD(qlm));
                else
                    tap1_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXDFE_ODD_TAP1_O_READ_VAL_6_0_RSVD(qlm));
                tap2_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXDFE_TAP2_O_READ_VAL_5_0_RSVD(qlm));
                tap3_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXDFE_TAP3_O_READ_VAL_4_0_RSVD(qlm));
                tap4_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXDFE_TAP4_O_READ_VAL_3_0_RSVD(qlm));
                tap5_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXDFE_TAP5_O_READ_VAL_3_0_RSVD(qlm));
                tap6_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXDFE_TAP6_O_READ_VAL_3_0_RSVD(qlm));
                tap7_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXDFE_TAP7_O_READ_VAL_2_0_RSVD(qlm));
                tap8_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN0_RXDFE_TAP8_O_READ_VAL_2_0_RSVD(qlm));
                break;
            case 1:
                if (use_even == 1)
                    tap1_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXDFE_EVEN_TAP1_O_READ_VAL_6_0_RSVD(qlm));
                else
                    tap1_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXDFE_ODD_TAP1_O_READ_VAL_6_0_RSVD(qlm));
                tap2_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXDFE_TAP2_O_READ_VAL_5_0_RSVD(qlm));
                tap3_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXDFE_TAP3_O_READ_VAL_4_0_RSVD(qlm));
                tap4_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXDFE_TAP4_O_READ_VAL_3_0_RSVD(qlm));
                tap5_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXDFE_TAP5_O_READ_VAL_3_0_RSVD(qlm));
                tap6_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXDFE_TAP6_O_READ_VAL_3_0_RSVD(qlm));
                tap7_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXDFE_TAP7_O_READ_VAL_2_0_RSVD(qlm));
                tap8_val = BDK_CSR_READ(node, BDK_GSERCX_PHY0_AFE_OVR_AFE_LN1_RXDFE_TAP8_O_READ_VAL_2_0_RSVD(qlm));
                break;
        }
...
{
    BDK_CSR_INIT(flag, node, BDK_GSERCX_PHY0_MB_CMD_FLAG(qlm));
    if (flag.s.data)
    {
        bdk_error("GSERC%d: Mailbox already has a command\n", qlm);
        return -1;
    }

    BDK_CSR_INIT(rsp_flag, node, BDK_GSERCX_PHY0_MB_RSP_FLAG(qlm));
    if (rsp_flag.s.data)
    {
        bdk_error("GSERC%d: Mailbox already has a response\n", qlm);
        return -1;
    }

    BDK_CSR_WRITE(node, BDK_GSERCX_PHY0_MB_CMD_DATA0(qlm), bdk_extract(args, 0, 8));
    BDK_CSR_WRITE(node, BDK_GSERCX_PHY0_MB_CMD_DATA1(qlm), bdk_extract(args, 8, 8));
    BDK_CSR_WRITE(node, BDK_GSERCX_PHY0_MB_CMD_DATA2(qlm), bdk_extract(args, 16, 8));
    BDK_CSR_WRITE(node, BDK_GSERCX_PHY0_MB_CMD_DATA3(qlm), bdk_extract(args, 24, 8));
    BDK_CSR_WRITE(node, BDK_GSERCX_PHY0_MB_CMD_DATA4(qlm), bdk_extract(args, 32, 8));
    BDK_CSR_WRITE(node, BDK_GSERCX_PHY0_MB_CMD_DATA5(qlm), bdk_extract(args, 40, 8));
    BDK_CSR_WRITE(node, BDK_GSERCX_PHY0_MB_CMD_DATA6(qlm), bdk_extract(args, 48, 8));
    BDK_CSR_WRITE(node, BDK_GSERCX_PHY0_MB_CMD_DATA7(qlm), bdk_extract(args, 56, 8));
    BDK_CSR_WRITE(node, BDK_GSERCX_PHY0_MB_CMD(qlm), cmd);

    if (BDK_CSR_WAIT_FOR_FIELD(node, BDK_GSERCX_PHY0_MB_CMD_FLAG(qlm), data, ==, 0, 10000))
    {
        bdk_error("GSERC%d: Mailbox stuck on command\n", qlm);
        return -1;
    }
    return 0;
}
...
{
    /* Wait for a response */
    if (BDK_CSR_WAIT_FOR_FIELD(node, BDK_GSERCX_PHY0_MB_RSP_FLAG(qlm), data, ==, 1, 10000))
    {
        bdk_error("GSERC%d: Mailbox has no response\n", qlm);
        return -1;
    }

    /* Read the response */
    BDK_CSR_INIT(rsp, node, BDK_GSERCX_PHY0_MB_RSP(qlm));
    BDK_CSR_INIT(data0, node, BDK_GSERCX_PHY0_MB_RSP_DATA0(qlm));
    BDK_CSR_INIT(data1, node, BDK_GSERCX_PHY0_MB_RSP_DATA1(qlm));
    BDK_CSR_INIT(data2, node, BDK_GSERCX_PHY0_MB_RSP_DATA2(qlm));
    BDK_CSR_INIT(data3, node, BDK_GSERCX_PHY0_MB_RSP_DATA3(qlm));
    BDK_CSR_INIT(data4, node, BDK_GSERCX_PHY0_MB_RSP_DATA4(qlm));
    BDK_CSR_INIT(data5, node, BDK_GSERCX_PHY0_MB_RSP_DATA5(qlm));
    BDK_CSR_INIT(data6, node, BDK_GSERCX_PHY0_MB_RSP_DATA6(qlm));
    BDK_CSR_INIT(data7, node, BDK_GSERCX_PHY0_MB_RSP_DATA7(qlm));
    BDK_CSR_INIT(data8, node, BDK_GSERCX_PHY0_MB_RSP_DATA8(qlm));
    BDK_CSR_INIT(data9, node, BDK_GSERCX_PHY0_MB_RSP_DATA9(qlm));
    BDK_CSR_INIT(data10, node, BDK_GSERCX_PHY0_MB_RSP_DATA10(qlm));
    BDK_CSR_INIT(data11, node, BDK_GSERCX_PHY0_MB_RSP_DATA11(qlm));
    BDK_CSR_INIT(data12, node, BDK_GSERCX_PHY0_MB_RSP_DATA12(qlm));
    BDK_CSR_INIT(data13, node, BDK_GSERCX_PHY0_MB_RSP_DATA13(qlm));
    BDK_CSR_INIT(data14, node, BDK_GSERCX_PHY0_MB_RSP_DATA14(qlm));
    BDK_CSR_INIT(data15, node, BDK_GSERCX_PHY0_MB_RSP_DATA15(qlm));

    /* Tell firmware we read the response */
    BDK_CSR_WRITE(node, BDK_GSERCX_PHY0_MB_RSP_FLAG(qlm), 1);
 ...
 */
int __bdk_qlm__gserc_rx_equalization(bdk_node_t node, int qlm, int qlm_lane)
{
    ...
    for (int lane = 0; lane < gsercx_const.s.nr_lanes; lane++)
    {
        ...
        BDK_TRACE(QLM, "N%d.GSERC%d.%d: Lane TXDP Clock Phase Calibration starting\n", node, qlm, lane);
        if (mailbox_command(node, qlm, 0x81, qlm_lane))
        {
            bdk_error("N%d.GSERC%d.%d: Lane TXDP Clock Phase Calibration failed to issue\n", node, qlm, lane);
            result = -1;
            continue;
        }
        uint64_t arg0 = 0;
        ...
        switch (status)
        {
            case 0x00: /* Calibration OK */
                BDK_TRACE(QLM, "N%d.GSERC%d.%d: Lane TXDP Clock Phase Calibration: %lu\n", node, qlm, lane, arg0 & 0xff);
                break;
            case 0x01: /* Calibration Railed Low */
                bdk_error("N%d.GSERC%d.%d: Lane TXDP Clock Phase Calibration Railed Low\n", node, qlm, lane);
                result = -1;
                break;
            case 0x02: /* Calibration Railed High */
                bdk_error("N%d.GSERC%d.%d: Lane TXDP Clock Phase Calibration Railed High\n", node, qlm, lane);
                result = -1;
                break;
            default:
                bdk_error("N%d.GSERC%d.%d: Lane TXDP Clock Phase Calibration illegal response %d\n", node, qlm, lane, status);
                result = -1;
                break;
        }
    }
 ...
 * @return Zero on success, negative on failure
 */
int __bdk_qlm_gserc_eye_capture(bdk_node_t node, int qlm, int lane, int show_data, bdk_qlm_eye_t *eye_data)
{
    int x_min = 0;
    int x_max = 127;
    int x_step = 1;
    ...
    {
        bdk_error("GSERC%d: Eye too high\n", qlm);
        return -1;
    }
    ...
    {
        bdk_error("GSERC%d: Eye setup 1 failed\n", qlm);
        return -1;
    }
    ...
    if (mailbox_response(node, qlm, &resp0, &resp1) != 0)
    {
        bdk_error("GSERC%d: Eye setup 2 failed\n", qlm);
        return -1;
    }
    ...
    while (rsp != 1)
    {
        ...
        if ((rsp != 0) || (rsp != 1))
        {
            bdk_error("GSERC%d: Eye failed (%d)\n", qlm, rsp);
            return -1;
        }
        ...
        for (int bits = 0; bits < 128; bits += ber)
        {
            ...
            if (y >= y_points)
            {
                ...
                if (x >= x_points)
                {
                    if (rsp != 1)
                    {
                        bdk_error("GSERC%d: Too many response\n", qlm);
                        return -1;
                    }
                    break; /* Bit loop */
                }
 ...
 */
void __bdk_qlm_gserc_init(bdk_node_t node)
{
    ...
    {
        bdk_error("GSERC: Unrecognized chip\n");
        return;
    }

    BDK_CSR_INIT(gsercx_init_ctl, node, BDK_GSERCX_COMMON_PHY_CTRL_BCFG(0));
    if (!gsercx_init_ctl.s.cpu_reset)
    {
        BDK_TRACE(QLM, "GSERC: Firmware already loaded\n");
        return;
    }

    /* Open the firmware file */
    BDK_TRACE(QLM, "GSERC: Loading firmware %s\n", firmware_file);
    uint64_t filesize = 0;
    uint64_t *firmware = bdk_signed_load(firmware_file, 0,BDK_SIGNED_IMAGE, BDK_SIGNED_FLAG_NOT_ENCRYPTED, &filesize);
    if (!firmware)
    {
        bdk_error("GSERC: Failed to load firmware %s\n", firmware_file);
        return;
    }
    ...
    for (int index = 0; index < (int)filesize / 8; index++)
    {
        for (int gserc = 0; gserc < num_gserc; gserc++)
            BDK_CSR_WRITE(node, BDK_GSERCX_PMEMX(gserc, index), firmware[index]);
    }
    free(firmware);
    BDK_TRACE(QLM, "GSERC: Loading firmware of %lu bytes\n", filesize);
    ...
    for (int gserc = 0; gserc < num_gserc; gserc++)
    {
        ...
        {
            bdk_qlm_state_lane_t state = __bdk_qlm_gserc_get_state(node, gserc, lane);
        ...
        for (int lane = 0; lane < num_lanes; lane++)
        {
            ...
            BDK_CSR_MODIFY(c, node, BDK_GSERCX_LANEX_CONTROL_BCFG(gserc, lane),
    ...
    for (int gserc = 0; gserc < num_gserc; gserc++)
    {
        ...
        BDK_CSR_MODIFY(c, node, BDK_GSERCX_COMMON_PHY_CTRL_BCFG(gserc),
            c.s.refclk_a_oe_r = 1;
    ...
    if (!bdk_is_platform(BDK_PLATFORM_ASIM))
    {
        for (int gserc = 0; gserc < num_gserc; gserc++)
        {
            if (BDK_CSR_WAIT_FOR_FIELD(node, BDK_GSERCX_COMMON_PHY_STATUS_BSTS(gserc), cm0_ok, ==, 1, 10000))
                bdk_error("GSERC%d: Timeout waiting for GSERCX_COMMON_PHY_STATUS_BSTS[cm0_ok]=1\n", gserc);
            if (BDK_CSR_WAIT_FOR_FIELD(node, BDK_GSERCX_COMMON_PHY_STATUS_BSTS(gserc), cm0_state_chng_rdy, ==, 1, 10000))
                bdk_error("GSERC%d: Timeout waiting for GSERCX_COMMON_PHY_STATUS_BSTS[cm0_state_chng_rdy]=1\n", gserc);
            if (BDK_CSR_WAIT_FOR_FIELD(node, BDK_GSERCX_COMMON_PHY_STATUS_BSTS(gserc), phy_error, ==, 0, 10000))
                bdk_error("GSERC%d: Timeout waiting for GSERCX_COMMON_PHY_STATUS_BSTS[phy_error]=0\n", gserc);
        }
    }
    ...
    if (!bdk_is_platform(BDK_PLATFORM_ASIM))
    {
        for (int gserc = 0; gserc < num_gserc; gserc++)
        {
            for (int lane = 0; lane < num_lanes; lane++)
            {
                if (BDK_CSR_WAIT_FOR_FIELD(node, BDK_GSERCX_LANEX_STATUS_BSTS(gserc, lane), ln_state_chng_rdy, ==, 1, 10000))
                    bdk_error("GSERC%d: Timeout waiting for GSERCX_LANEX_STATUS_BSTS[ln_state_chng_rdy]=1\n", gserc);
            }
            /* Enable the BIST/PRBS clocks before starting each lane */
            BDK_CSR_MODIFY(c, node, BDK_GSERCX_PHY0_TOP_CLOCK_LN0_CG_CTRL(gserc),
                c.s.bist_rx = 1;
                c.s.bist_tx = 1);
            BDK_CSR_MODIFY(c, node, BDK_GSERCX_PHY0_TOP_CLOCK_LN1_CG_CTRL(gserc),
                c.s.bist_rx = 1;
                c.s.bist_tx = 1);
        }
    }
    ...
    if (!bdk_is_platform(BDK_PLATFORM_ASIM))
    {
        for (int gserc = 0; gserc < num_gserc; gserc++)
        {
            for (int lane = 0; lane < num_lanes; lane++)
            {
                if (BDK_CSR_WAIT_FOR_FIELD(node, BDK_GSERCX_LANEX_STATUS_BSTS(gserc, lane), ln_tx_rdy, ==, 1, 10000))
                    bdk_error("GSERC%d: Timeout waiting for GSERCX_LANEX_STATUS_BSTS[ln_tx_rdy]=1\n", gserc);
                if (BDK_CSR_WAIT_FOR_FIELD(node, BDK_GSERCX_LANEX_STATUS_BSTS(gserc, lane), ln_rx_rdy, ==, 1, 10000))
                    bdk_error("GSERC%d: Timeout waiting for GSERCX_LANEX_STATUS_BSTS[ln_rx_rdy]=1\n", gserc);
                if (BDK_CSR_WAIT_FOR_FIELD(node, BDK_GSERCX_LANEX_STATUS_BSTS(gserc, lane), ln_state_chng_rdy, ==, 1, 10000))
                    bdk_error("GSERC%d: Timeout waiting for GSERCX_LANEX_STATUS_BSTS[ln_state_chng_rdy]=1\n", gserc);
            }
        }
