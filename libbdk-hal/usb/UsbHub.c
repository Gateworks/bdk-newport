  ...
  )
{
  ...
  if (Result != EFI_USB_NOERROR) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG (( EFI_D_ERROR, "failed to remove async transfer - %d\n", (int) Status));
      return Status;
    }
    ...
    if (EFI_ERROR (Status)) {
      DEBUG (( EFI_D_ERROR, "failed to submit new async transfer - %d\n", (int) Status));
    }
  ...
  )
{
  ...
  if (Index == NumEndpoints) {
    DEBUG (( EFI_D_ERROR, "no interrupt endpoint found for hub %d\n", HubDev->Address));
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (( EFI_D_ERROR, "failed to read HUB descriptor %d\n", (int) Status));
    return Status;
  }

  HubIf->NumOfPort = HubDesc.NumPorts;

  DEBUG (( EFI_D_INFO, "hub %d has %d ports\n", HubDev->Address,HubIf->NumOfPort));
  ...
  if (HubIf->Device->Speed == EFI_USB_SPEED_SUPER) {
    ...
    DEBUG ((EFI_D_INFO, "Set Hub Depth as 0x%x\n", Depth));
...
#else
      bdk_wait_usec(HubDesc.PwrOn2PwrGood * USB_SET_PORT_POWER_STALL);
#endif
    }
  ...
  if (EFI_ERROR (Status)) {
      DEBUG (( EFI_D_ERROR, "failed to create signal for hub %d - %d\n",
      ...
      BDK_WMB;
      DEBUG (( EFI_D_INFO, "Created Notify Interface for hub %d @%p\n",
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (( EFI_D_ERROR, "failed to queue interrupt transfer for hub %d - %d\n",
                HubDev->Address, (int) Status));
#if defined(notdef_cavium)
  ...
  DEBUG (( EFI_D_INFO, "hub %d initialized\n", HubDev->Address));
  return Status;
}
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG (( EFI_D_INFO, "skip reset on hub %p port %d\n", HubIf, Port));
    return EFI_SUCCESS;
  }
...
#else
  bdk_wait_usec(USB_SET_PORT_RESET_STALL);
#endif
  //
...
#else
      bdk_wait_usec(USB_SET_PORT_RECOVERY_STALL);
#endif
      return EFI_SUCCESS;
    }
#if defined(notdef_cavium)
...
#else
    bdk_wait_usec(USB_WAIT_PORT_STS_CHANGE_STALL);
#endif
  }
  ...
  )
{
  ...
  BDK_WMB;
#endif
  DEBUG (( EFI_D_INFO, "hub device %d released\n", HubIf->Device->Address));
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  DEBUG (( EFI_D_INFO, "root hub %p - max speed %d, %d ports\n",
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG (( EFI_D_INFO, "skip reset on root port %d\n", Port));
    return EFI_SUCCESS;
  }
  ...
  if (EFI_ERROR (Status)) {
      DEBUG (( EFI_D_ERROR, "failed to start reset on port %d status %d\n", Port, (int) Status));
    return Status;
  }
...
#else
  bdk_wait_usec(USB_SET_ROOT_PORT_RESET_STALL);
#endif
  Status = UsbHcClearRootHubPortFeature (Bus, Port, EfiUsbPortReset);

  if (EFI_ERROR (Status)) {
    DEBUG (( EFI_D_ERROR, "failed to clear reset on port %d\n", Port));
    return Status;
  }
#if defined(notdef_cavium)
  gBS->Stall (USB_CLR_ROOT_PORT_RESET_STALL);
#else
  bdk_wait_usec(USB_CLR_ROOT_PORT_RESET_STALL);
#endif
  ...
  if (Index == USB_WAIT_PORT_STS_CHANGE_LOOP) {
    DEBUG ((EFI_D_ERROR, "reset not finished in time on port %d\n", Port));
    return EFI_TIMEOUT;
  }
  ...
  if (!USB_BIT_IS_SET (PortState.PortStatus, USB_PORT_STAT_ENABLE)) {
    ...
    if (RootIf->MaxSpeed == EFI_USB_SPEED_HIGH) {
      DEBUG (( EFI_D_ERROR, "release low/full speed device (%d) to UHCI\n", Port));
      ...
      if (EFI_ERROR (Status)) {
        DEBUG (( EFI_D_ERROR, "failed to enable port %d for UHCI\n", Port));
        return Status;
      }
#if defined(notdef_cavium)
      gBS->Stall (USB_SET_ROOT_PORT_ENABLE_STALL);
#else
      bdk_wait_usec(USB_SET_ROOT_PORT_ENABLE_STALL);
#endif
    }
  }
...
{
#if defined(notdef_cavium)
  DEBUG (( EFI_D_INFO, "root hub released for hub %p\n", HubIf));
