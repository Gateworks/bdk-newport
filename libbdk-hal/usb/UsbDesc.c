  ...
  )
{
  ...
  *Consumed = 0;
  ...
  default:
    DEBUG ((EFI_D_ERROR, "Unknown descriptor type = 0x%02x!\n", (int) Type));
 //   ASSERT (FALSE);
    return NULL;
  }
  ...
  if (Len <= sizeof (USB_DESC_HEAD)) {
    DEBUG ((EFI_D_ERROR, "met mal-format descriptor, total length = %d!\n", (int)Len));
    return NULL;
  }
  ...
  while (Offset < Len - sizeof (USB_DESC_HEAD)) {
    ...
    if (Head->Len == 0) {
      DEBUG ((EFI_D_ERROR, "met mal-format descriptor, Head->Len = 0!\n"));
      return NULL;
    }
    ...
    if (Head->Len > MAX_UINTN - Offset) {
      DEBUG ((EFI_D_ERROR, "met mal-format descriptor, Head->Len = %d!\n", Head->Len));
      return NULL;
    }
  ...
  //
  if (Len < Offset) {
    DEBUG ((EFI_D_ERROR, "UsbCreateDesc: met mal-format descriptor, Offset/Len = %lld/%lld!\n", Offset, Len));
    return NULL;
  }

  if ((Head->Type != Type) || (Head->Len < DescLen)) {
    DEBUG ((EFI_D_ERROR, "UsbCreateDesc: descriptor cannot be found, Header(T/L) = %d/%d!\n", Head->Type, Head->Len));
    return NULL;
  }
  ...
  )
{
  ...
  *Consumed = 0;
  ...
  if (Setting == NULL) {
    DEBUG (( EFI_D_ERROR, "failed to create interface descriptor\n"));
    return NULL;
  }
  ...
  DEBUG (( EFI_D_INFO, "interface %d(setting %d) has %d endpoints\n",
  ...
  for (Index = 0; (Index < NumEp) && (Offset < Len); Index++) {
    ...
    if (Ep == NULL) {
      DEBUG (( EFI_D_ERROR, "failed to create endpoint(index %d)\n", (UINT32)Index));
      goto ON_ERROR;
    }
  ...
  )
{
  ...
  DEBUG (( EFI_D_INFO, "config %d has %d interfaces\n",
  ...
  while (Len >= sizeof (EFI_USB_INTERFACE_DESCRIPTOR)
    ...
    if (Setting == NULL) {
      DEBUG (( EFI_D_ERROR, "warning: failed to get interface setting, stop parsing now.\n"));
      break;

    } else if (Setting->Desc.InterfaceNumber >= NumIf) {
      DEBUG (( EFI_D_ERROR, "mal-formated interface descriptor\n"));
...
#else
    bdk_wait_usec(USB_RETRY_MAX_PACK_SIZE_STALL);
#endif
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (( EFI_D_ERROR, "failed to get descript length(%d) %d\n",
                Desc.TotalLength, (int) Status));

    return NULL;
  }

  DEBUG (( EFI_D_INFO, "total length is %d\n", Desc.TotalLength));
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (( EFI_D_ERROR, "failed to get full descript %d\n", (int) Status));
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (( EFI_D_ERROR, "failed to get device descriptor - %d\n", (int) Status));
    return Status;
  }
  ...
  DEBUG (( EFI_D_INFO, "device has %d configures\n", NumConfig));
  ...
  for (Index = 0; Index < NumConfig; Index++) {
    ...
    if (Config == NULL) {
      DEBUG (( EFI_D_ERROR, "failed to get configure (index %d)\n", Index));
    ...
    if (ConfigDesc == NULL) {
      DEBUG (( EFI_D_ERROR, "failed to parse configure (index %d)\n", Index));
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (( EFI_D_INFO, "get language ID table %d\n", (int) Status));
  }
