...
xhci_t* createUsbXHci(bdk_node_t node, int usb_port)
{
    ...
    if (NULL != (dev = bdk_device_lookup(node, BDK_DEVICE_DEVID(BDK_PCC_DEV_IDL_E_USBDRD), usb_port)) )  {
    ...
    BDK_CSR_DEFINE(caplength, BDK_USBDRDX_UAHC_CAPLENGTH(0));
    ...
    BDK_CSR_DEFINE(pgsz, BDK_USBDRDX_UAHC_PAGESIZE(0));
  ...
  )
{
  ...
  *TransferResult = EFI_USB_ERR_SYSTEM;
  ...
  if (SlotId == 0) {
    DEBUG ((EFI_D_ERROR, "Device is disabled\n"));
    goto ON_EXIT;
  }
  ...
  if ((Request->Request     == USB_REQ_SET_ADDRESS) &&
    ...
    if (Xhc->UsbDevContext[SlotId].XhciDevAddr == 0) {
      DEBUG ((EFI_D_ERROR, "Attempting to set address to 0\n"));
      Status = EFI_DEVICE_ERROR;
      goto ON_EXIT;
    }
    //
