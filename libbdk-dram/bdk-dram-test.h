...
// Choose prediction-based algorithms for mem_xor and mem_rows tests (bdk-dram-test-patfil.c)
// Change to 0 to go back to the original versions.
#define USE_PREDICTION_CODE_VERSIONS 1
 ...
 * @param rank    pointer to Physical RANK on the LMC for return value
 *
 * @return LMC controller the address was for
 *
 * NOTE: assumptions made to speed up processing:
 *       -  LMC number is always aliased
 */
extern int
bdk_dram_address_extract_lmc_rank_cn8xxx(uint64_t address, int xbits, int *rank);
 ...
 * @param bitno   Bit to corrupt in the byte (0-7), or -1 to create a double bit fault in the ECC
 *                byte.
 */
extern void bdk_dram_test_inject_error(int etype, uint64_t address, int bitno);
...
extern void __bdk_dram_invalidate_mem(uint64_t address);
...
extern int  __bdk_dram_retry_failure2(int burst, uint64_t address1, uint64_t data1,
