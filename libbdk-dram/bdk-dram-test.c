...
static void bdk_dram_probe_cores(bdk_node_t node);
...
/**
 * Force the memory at the pointer location to be evicted
 * from L2 without write-back. L1 will be unaffected.
 *
 * @param address Physical memory location
 */
void __bdk_dram_invalidate_mem(uint64_t address)
{
    BDK_MB;
...
{
    /* Figure out the address of the byte one off the top of memory */
    ...
    for (bdk_node_t node = BDK_NODE_0; node < BDK_NUMA_MAX_NODES; node++)
    {
        if (flags & (1 << node))
        {
            ...
            {
                if (__bdk_dram_is_lmc_in_dreset(node, i))
                {
                    start_dram_dclk[node][i] = 0;
                    start_dram_ops[node][i] = 0;
                }
                else
                {
    ...
    while ((work_address < end_address) && ((dram_test_thread_errors == 0) || (flags & BDK_DRAM_TEST_NO_STOP_ERROR)))
    {
        ...
        // FIXME: round size to 2 x cache block size, so that when there are 2 areas,
        //  each area size is an integral number of cache blocks
        thread_size += 255;  // was 127
        thread_size &= -256; // was 128
...
#endif
    } /* while ((work_address < end_address) etc) */

    // One final yield to the error polling thread,
    // to try to catch all the test-related ECC errs.
    bdk_thread_yield();
    ...
    for (bdk_node_t node = BDK_NODE_0; node < BDK_NUMA_MAX_NODES; node++)
    {
        if (flags & (1 << node))
        {
            ...
            for (int i = 0; i < num_dram_controllers; i++)
            {
                if (__bdk_dram_is_lmc_in_dreset(node, i))
                {
                    stop_dram_dclk[node][i] = 0;
                    stop_dram_ops[node][i] = 0;
                }
                else
                {
...
int bdk_dram_test(int test, uint64_t start_address, uint64_t length, bdk_dram_test_flags_t flags)
{
    ...
    if ((flags & bdk_build_mask(BDK_NUMA_MAX_NODES)) == 0)
        flags |= bdk_build_mask(BDK_NUMA_MAX_NODES);
 ...
 *
 */
static void __bdk_dram_report_address_decode(uint64_t address, uint64_t orig_xor, char *buffer, int len)
{
    ...
    if (bdk_is_model(OCTEONTX_CN9XXX)) {
        ...
        if (lmcaddr == (uint64_t)-1)
            printf("%s: ASC Address Conversion Failed\n", __func__);
    } else
        lmcaddr = address;

    bdk_dram_address_extract_info(lmcaddr, &node, &lmc, &dimm, &prank, &lrank, &bank, &row, &col);

    snprintf(buffer, len, "N%d.LMC%d: CMP byte %d xor 0x%02lx (DIMM%d,Rank%d/%d,Bank%02d,Row 0x%05x,Col 0x%04x)<PA=0x%012lx>",
 ...
 *
 * @param physaddr Physical address the error occurred at
 ...
 */
void __bdk_dram_report_error(uint64_t physaddr, uint64_t data, uint64_t correct, int burst, int fails)
{
    char buffer[128];
    char failbuf[32];
	...
	__bdk_dram_report_address_decode(physaddr, xor, buffer, sizeof(buffer));
        bdk_error("%s%s\n", buffer, failbuf);
...
int __bdk_dram_retry_failure(int burst, uint64_t address, uint64_t data, uint64_t expected)
{
	...
	for (int i = 0; i < RETRY_LIMIT; i++) {
	    ...
	    __bdk_dram_invalidate_mem(address);
    ...
    // If we failed on every retry, it really was bad in memory, so try to re-write it.
    // This will try to restore what the prediction code expects in memory, so later bursts
    // will hopefully be working with good data...
    if (refail == RETRY_LIMIT) {
        __bdk_dram_write64(address, expected);
 ...
 * retry_failure2
 *
 * Used when two addresses are involved in the failure.
 *
 * @param burst
 * @param address1
 * @param address2
 */
int __bdk_dram_retry_failure2(int burst, uint64_t address1, uint64_t data1,
                              uint64_t address2, uint64_t data2)
{
    int refail = 0;
	...
	for (int i = 0; i < RETRY_LIMIT; i++) {
	    __bdk_dram_invalidate_mem(address1);
	    __bdk_dram_invalidate_mem(address2);
	    BDK_DCACHE_INVALIDATE;
    ...
    // NOTE: We do not know which datum is wrong, so report both.
    //       If there is an accompanying ECC error, the real failing address is there!!!
    // FIXME: these will increment the errors always by 1 too many, but maybe not print...
    __bdk_dram_report_error(address1, data1, data2, burst, refail);
    __bdk_dram_report_error(address2, data2, data1, burst, refail);
 ...
 *
 * @param etype   Error type: 1 = SBE, 2 = DBE, 3 = PARITY
 * @param address For DBE/SBE: Physical address to corrupt. Any byte alignment is supported
 *                For PARITY: LMC number (0-3)
 * @param bitno   For DBE/SBE: Bit to corrupt in the byte (0-7), or -1 to create a double bit fault in the ECC byte.
 *                For PARITY: Type of DDR command to corrupt (0x00-0x15)
 */
extern void
ddr4_mrw(bdk_node_t node, int ddr_interface_num, int rank, int mr_wr_addr, int mr_wr_sel, int mr_wr_bg1);

void bdk_dram_test_inject_error(int etype, uint64_t address, int bitno)
{
    if (etype == 3) { // PARITY error inject
        int node = bdk_numa_local();

        int lmc = address & 3;
        // FIXME: validate LMC number!!!

        printf("Injecting PARITY error on LMC%1d, CA_PARITY_SEL = 0x%x\n", lmc, bitno);

        BDK_CSR_MODIFY(c, node, BDK_LMCX_ECC_PARITY_TEST(lmc),
                       c.s.ca_parity_sel = bitno;
                       c.s.ca_parity_corrupt_ena = 1);
        BDK_CSR_READ(node, BDK_LMCX_ECC_PARITY_TEST(lmc));

        bdk_wait_usec(100000); // 100 msecs
        ddr4_mrw(node, lmc, /*rank*/0, /*defaults*/-1, /*MR3*/3, 0);
        bdk_wait_usec(100000); // 100 msecs

        BDK_CSR_MODIFY(c, node, BDK_LMCX_ECC_PARITY_TEST(lmc),
                       c.s.ca_parity_sel = 0;
                       c.s.ca_parity_corrupt_ena = 0);
        BDK_CSR_READ(node, BDK_LMCX_ECC_PARITY_TEST(lmc));
    ...
    int corrupt_bit = -1;
    if (bitno >= 0)
        corrupt_bit = (address & 0xf) * 8 + bitno;
    ...
    if (bdk_is_model(OCTEONTX_CN9XXX)) {
        ...
        lmcaddr = bdk_ccs_convert_pa_to_lmc(node, address);
        if (lmcaddr == (uint64_t)-1) {
            printf("%s: ERROR: ASC Address Conversion Failed of 0x%012lx\n",
                   __func__, address);
            return;
        }
    } else {
        node = bdk_extract(address, 42, 2);
        lmcaddr = address;
    }
    bdk_dram_address_extract_info(lmcaddr, &node, &lmc, &dimm, &prank, &lrank, &bank, &row, &col);
    ...
    if ((corrupt_bit >= 0) && (corrupt_bit < 64))
        ...
        BDK_CSR_WRITE(node, BDK_LMCX_CHAR_MASK0(lmc), (bitno == -1) ? 3 : 0);

    BDK_CSR_WRITE(node, BDK_LMCX_CHAR_MASK2(lmc),
                  (corrupt_bit >= 64) ? (1ull << (corrupt_bit - 64)) : 0);
...
/**
 * Probe all cores for Dcache and Icache parity errors
 *
 * @param node     Node to set
 */
#include "libbdk-arch/bdk-csrs-ap.h"
//                        L1DPERR       MTLBPERR       UTLBPERR
#define DCACHE_ERR_BITS ((1ull << 8) | (1ull << 12) | (1ull << 16))
...
static void bdk_dram_probe_cores(bdk_node_t node)
{
    ...
    if (bdk_is_platform(BDK_PLATFORM_ASIM))
        return;
    /* FIXME: This code needs update for CN9XXX */
    if (bdk_is_model(OCTEONTX_CN9XXX))
        return;
    ...
    for (core = 0; core < num_cores; core++) {
        ...
        if (running & (1ull << core)) {
            ...
            // look for Dcache errors
            BDK_CSR_DEFINE(errm, BDK_AP_CVM_ERRMEM_EL1);
            errm.u = bdk_sysreg_read(node, core, BDK_AP_CVM_ERRMEM_EL1);
            if (errm.u & DCACHE_ERR_BITS) {
                printf("N%d.CORE%d: EVENT: AP_CVM_ERRMEM_EL1 0x%016lx\n",
                       node, core, errm.u);
                errm.u &= ~DCACHE_ERR_BITS;
                bdk_sysreg_write(node, core, BDK_AP_CVM_ERRMEM_EL1, errm.u);
            }

            // look at Icache error
            BDK_CSR_DEFINE(erri, BDK_AP_CVM_ERRICACHE_EL1);
            erri.u = bdk_sysreg_read(node, core, BDK_AP_CVM_ERRICACHE_EL1);
            if (erri.s.icache_data_error) {
                printf("N%d.CORE%d: EVENT: AP_CVM_ERRICACHE_EL1 0x%016lx\n",
                       node, core, erri.u);
                erri.s.icache_data_error = 0;
                bdk_sysreg_write(node, core, BDK_AP_CVM_ERRICACHE_EL1, erri.u);
            }
        }
    }
}
