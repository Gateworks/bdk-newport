...
typedef enum {
   ...
   BDK_CSR_TYPE_MDSB,           /**< CN96XX: Memory Diagnostic Serial Bus?, not memory mapped */
   ...
   BDK_CSR_TYPE_PCICONFIGEP_SHADOW, /**< CN96XX: PCIEP register invisible to host, not memory mapped */
   BDK_CSR_TYPE_PCICONFIGEPVF,  /**< CN96XX: PCIEP registers only on vertain PEMs, not memory mapped */
...
typedef struct __attribute__ ((packed)) {
    ...
    uint16_t        range_index; /**< Index into __bdk_csr_db_range where the range is */
    uint16_t        param_inc_index; /**< Index into __bdk_csr_db_number where the param multiplier is */
} __bdk_csr_db_type_t;
...
extern void __bdk_csr_fatal(const char *name, int num_args, unsigned long arg1, unsigned long arg2, unsigned long arg3, unsigned long arg4, unsigned long arg5, unsigned long arg6) __attribute__ ((noreturn));
extern int bdk_csr_decode_indexed(int db_index, int params[BDK_CSR_DB_MAX_PARAM], uint64_t value);
extern int bdk_csr_decode(const char *name, uint64_t value);
extern int bdk_csr_decode_by_address(uint64_t address, uint64_t value);
extern int bdk_csr_field(const char *csr_name, int field_start_bit, const char **field_name);
extern uint64_t bdk_csr_read_indexed(bdk_node_t node, int db_index, int params[BDK_CSR_DB_MAX_PARAM]);
...
extern uint64_t bdk_csr_load_add_by_name(bdk_node_t node, const char *name, uint64_t value);
extern int __bdk_csr_lookup_index(const char *name, int params[]);
extern int bdk_csr_get_name(const char *last_name, char *buffer, bool use_prefix);
