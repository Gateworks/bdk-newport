...
union bdk_rom_clib_s
{
    ...
    struct bdk_rom_clib_s_s
    {
        ...
        uint64_t rom_script_chksum     : 32; /**< [351:320] 32-bit checksum of the ROM script, calculated as 32-bit ADD across all 32-bit words in the image. */
...
#endif /* Word 1 - End */
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 2 - Big Endian */
        uint64_t offset                : 64; /**< [191:128] Offset address. The flash address for first byte of SCP BL1 image.
                                                                 Must be 1 KB aligned, and the entire BL1 image must be in the lower 16 MB of the flash.
...
#endif /* Word 2 - End */
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 3 - Big Endian */
        uint64_t size                  : 64; /**< [255:192] Size. The size of the SCP_BL1 FW material in bytes.  Must be multiple of 16 KB.
                                                                 Maximum value is (CPC_CONST[MEM_REGIONS]-1)*16 KB, which in CNXXXX is 368 KB. */
#else /* Word 3 - Little Endian */
        uint64_t size                  : 64; /**< [255:192] Size. The size of the SCP_BL1 FW material in bytes.  Must be multiple of 16 KB.
                                                                 Maximum value is (CPC_CONST[MEM_REGIONS]-1)*16 KB, which in CNXXXX is 368 KB. */
#endif /* Word 3 - End */
...
union bdk_rom_csib_s
{
    ...
    /* struct bdk_rom_csib_s_cn9 cn96xxp1; */
    struct bdk_rom_csib_s_cn96xxp3
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t magic                 : 64; /**< [ 63:  0] Magic number.  Contains "CVM_CSIB" in ASCII (0x42495343_5f4d5643)
                                                                 to indicate a valid ROM_CSIB_S structure. */
    ...
    /* struct bdk_rom_csib_s_cn96xxp3 cn98xx; */
    /* struct bdk_rom_csib_s_cn96xxp3 cnf95xx; */
    /* struct bdk_rom_csib_s_cn96xxp3 loki; */
};
