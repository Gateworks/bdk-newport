...
int __bdk_fs_init_early(void)
{
    ...
    dev_uart0[6] = '0' + node;
    dev_uart1[6] = '0' + node;
    bdk_console_connect_fd(3); /* Uart 0 */
    /* CN8XXX allows both uarts. CN9XXX and beyond reserves uart 1 for SCP */
    if (bdk_is_model(OCTEONTX_CN8XXX))
        bdk_console_connect_fd(4); /* Uart 1 */
    return 0;
}
...
int __bdk_fs_init_late(void)
{
    ...
    if (__bdk_fs_base64_init)
        result |= __bdk_fs_base64_init();
