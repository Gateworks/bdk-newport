...
typedef struct bdk_thread
{
    ...
    bdk_thread_flags_t flags;
    uint64_t    stack_canary;
    uint64_t    stack[0] __attribute__ ((aligned (16))); /* SP must be 16 byte aligned */
} bdk_thread_t;
...
void bdk_thread_yield(void)
{
    ...
    /* SSO process might yield if tracing is turned on and the uart is busy. We
       set the flag BDK_THREAD_FLAG_IN_SSO to make sure we don't recurse if
       this happens. Recursion causes stack overflow */
    if (bdk_sso_process_work && !(current->flags & BDK_THREAD_FLAG_IN_SSO))
    {
        current->flags |= BDK_THREAD_FLAG_IN_SSO;
