...
/* MPI/SPI filenames are of the format:
   n0.mpi0/cs0-[hl],[12]wire,idle-[rhl],[ml]sb,##bit,<freq>
...
static int mpi_open(__bdk_fs_dev_t *handle, int flags)
{
    ...
    int address_width;
    int freq;
    int count = sscanf(handle->filename, "n%d.mpi%d/cs%d-%c,%cwire,idle-%c,%csb,%dbit,%d",
        &node, &mpi_block, &chip_sel, &chip_hl, &wire, &idle, &endian, &address_width, &freq);
    if (count != 9)
        goto bad;
    if (node != (int)handle->dev_node)
        goto bad;
    if (mpi_block != handle->dev_index)
        goto bad;
    ...
    if (bdk_mpi_initialize(node, mpi_block, freq * 1000000, mpi_flags))
        return -1;

    /* Write zero to status register to disable write protection */
    if ((flags&3) != O_RDONLY)
        bdk_mpi_transfer(node, mpi_block, chip_sel, 0, 2, CMD_WRITE_STATUS, 0);
...
static int get_chip_select(const char *name)
{
    int cs = name[15] - '0';
    if ((name[13] != 'c') || (name[14] != 's') || (cs < 0) || (cs >= 4))
        bdk_fatal("MPIFS: Incorrect filename %s\n", name);
    return cs;
}
 ...
 * SPI EEPROM, a SPI NOR, or a SPI NAND. All three use a common read command
 * and format.
 *
 * @param node      Node number
 * @param mpi_block MPI block to use
 * @param chip_sel  Chip select for device
 * @param address_width
 *                  Address width (24/32bits)
...
static int mpi_read_buffer(bdk_node_t node, int mpi_block, int chip_sel, int address_width,
                           uint64_t loc, void *buffer, int length)
{
    const int CMD_READ = 0x03;
    uint64_t tx_data;

    BDK_CSR_DEFINE(mpi_cfg, BDK_MPIX_CFG(mpi_block));
    if (bdk_is_model(OCTEONTX_CN8XXX))
        mpi_cfg.u = BDK_CSR_READ(bdk_numa_master(), BDK_MPI_CFG);
    else
        mpi_cfg.u = BDK_CSR_READ(bdk_numa_master(), BDK_MPIX_CFG(mpi_block));
    ...
    bdk_mpi_transfer(node, mpi_block, chip_sel, 1, 1 + address_width/8, tx_data, 0);
    int data_left = length;
    while (data_left > 0)
    {
        *(uint64_t*)buffer = bdk_swap64(bdk_mpi_transfer(node, mpi_block, chip_sel, data_left>8, 0, 0, 8));
        buffer+=8;
        data_left-=8;
    }
    return length;
...
static int mpi_read(__bdk_fs_dev_t *handle, void *buffer, int length)
{
    ...
    const int mpi_block = handle->dev_index;
    const int chip_sel = get_chip_select(handle->filename);
    const int address_width = get_address_width(handle->filename);

    return mpi_read_buffer(node, mpi_block, chip_sel, address_width, handle->location, buffer, length);
}
 ...
 * @param mpi_block MPI block to query
 * @param chip_sel  Chip select the device is on
 *
 * @return Zero on success, negative on timeout
 */
static int wait_for_write_complete(bdk_node_t node, int mpi_block, int chip_sel)
{
    ...
    do
    {
        status = bdk_mpi_transfer(node, mpi_block, chip_sel, 0, 1, CMD_READ_STATUS, 1);
        if (status & 1)
        {
...
static int eeprom_write(__bdk_fs_dev_t *handle, const void *buffer, int length)
{
    ...
    while (data_left)
    {
        if (need_new_write)
        {
            if (wait_for_write_complete(node, mpi_block, chip_sel))
                return -1;
            /* Write enable */
            bdk_mpi_transfer(node, mpi_block, chip_sel, 0, 1, CMD_WRITE_EN, 0);
            /* Begin write */
            bdk_mpi_transfer(node, mpi_block, chip_sel, 1, 1, CMD_WRITE, 0);
            bdk_mpi_transfer(node, mpi_block, chip_sel, 1, address_width/8, loc, 0);
        }
        need_new_write = ((loc & 63) == 63) || (data_left == 1);
        bdk_mpi_transfer(node, mpi_block, chip_sel, !need_new_write, 1, *(uint8_t*)buffer, 0);
        buffer++;
        loc++;
        data_left--;
    }
...
static int nor_write(__bdk_fs_dev_t *handle, const void *buffer, int length)
{
    ...
    /* We only use a small subset of the commands support by the NOR */
    ...
    while (bytes_remain)
    {
        ...
        if ((0 != sector_offset) || (chunk < ERASE_SIZE))
        {
            ...
            mpi_read_buffer(node, mpi_block, chip_sel, address_width, sector_addr, sector_buf, ERASE_SIZE);
        ...
        bdk_mpi_transfer(node, mpi_block, chip_sel, 0, 1, CMD_WREN, 0);
        /* We need to erase the next block */
        bdk_mpi_transfer(node, mpi_block, chip_sel, 0, /* Disable chip select */
        ...
        while (data_left)
        {
            bdk_mpi_transfer(node, mpi_block, chip_sel, 0, 1, CMD_WREN, 0);
            /* Begin page program */
            bdk_mpi_transfer(node, mpi_block, chip_sel, 1, /* Keep chip select active */
                1 + address_width/8, /* One command byte plus address */
            ...
            while (len--)
            {
                ...
                bdk_mpi_transfer(node, mpi_block, chip_sel, (len != 0),
                    1, /* One data byte */
...
{
    int num_mpi = bdk_mpi_get_controllers(bdk_numa_master());
    for (int mpi = 0; mpi < num_mpi; mpi++)
    {
        if (bdk_fs_register_dev("mpi", mpi, &bdk_fs_mpi_ops))
            return -1;
    }
    return 0;
}
